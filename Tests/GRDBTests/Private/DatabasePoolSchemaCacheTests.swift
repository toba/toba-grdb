...
class DatabasePoolSchemaCacheTests: GRDBTestCase {
    func testCache() throws {
        ...
        try dbPool.write { db in
            try db.execute(
                sql: "CREATE TABLE items (id INTEGER PRIMARY KEY, email TEXT UNIQUE, foo INT, bar DOUBLE)"
            )
        ...
        try dbPool.read { db in
            do {
                // Assert that cache is used: we expect an error now that the cache is empty.
                _ = try db.primaryKey("items")
                XCTFail()
    ...
    func testCacheSnapshotIsolation() throws {
        // This test checks that the schema cache follows snapshot isolation. and that writer and
        // readers do not naively share the same cache.
        let dbPool = try makeDatabasePool()

        // writer reader CREATE TABLE foo table exists: true
        // >
        let s1 = DispatchSemaphore(value: 0)
        //                          table exists: true
        //                          <
        let s2 = DispatchSemaphore(value: 0)
        // DROP TABLE foo table exists: false
        // >
        ...
        let block2: @Sendable () -> Void = {
            _ = s1.wait(timeout: .distantFuture)
            try! dbPool.read { db in
                // activate snapshot isolation so that foo table is visible during the whole read.
                // Any read is enough.
