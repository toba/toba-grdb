...
class ValueObservationRowTests: GRDBTestCase {
    func testAll() throws {
        let request = SQLRequest<SendableRow>(sql: "SELECT * FROM t ORDER BY id")
    ...
    func testOne() throws {
        let request = SQLRequest<SendableRow>(sql: "SELECT * FROM t ORDER BY id DESC")
    ...
    func testFTS4Observation() throws {
        try assertValueObservation(
            ValueObservation.trackingConstantRegion(
                SQLRequest<SendableRow>(sql: "SELECT * FROM ft_documents").fetchAll),
            records: [[], [["content": "foo"]]],
            setup: { db in try db.create(virtualTable: "ft_documents", using: FTS4()) },
            recordedUpdates: { db in
    ...
    func testSynchronizedFTS4Observation() throws {
        try assertValueObservation(
            ...
            recordedUpdates: { db in
                try db.execute(
                    sql: "INSERT INTO documents (content) VALUES (?)", arguments: ["foo"])
            })
    }
    ...
    func testJoinedFTS4Observation() throws {
        try assertValueObservation(
            ...
            records: [[], [["id": 1]]],
            setup: { db in
                try db.create(table: "document") { t in t.autoIncrementedPrimaryKey("id") }
                try db.create(virtualTable: "ft_document", using: FTS4()) { t in t.column("content")
                }
            },
            recordedUpdates: { db in
                try db.inTransaction {
