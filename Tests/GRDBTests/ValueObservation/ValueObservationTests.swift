...
class ValueObservationTests: GRDBTestCase {
    // Test passes if it compiles. See <https://github.com/groue/GRDB.swift/issues/1541> start(in:)
    // resolves to the main-actor overload here
    @MainActor
    ...
    // Test passes if it compiles. See <https://github.com/groue/GRDB.swift/issues/1541>
    func testValuesFromAnyDatabaseWriter(writer: any DatabaseWriter) {
        func observe<T>(
            fetch: @escaping @Sendable (Database) throws -> T
        ) throws -> AsyncValueObservation<T> { ValueObservation.tracking(fetch).values(in: writer) }
    }
    ...
    func testImmediateError() throws {
        ...
        func test(_ dbWriter: some DatabaseWriter) throws {
            ...
            _ = observation.start(
                ...
                onError: { error in errorMutex.store(error as? TestError) },
                onChange: { _ in })
    ...
    func testErrorCompletesTheObservation() throws {
        ...
        func test(_ dbWriter: some DatabaseWriter) throws {
            ...
            let observation = ValueObservation.trackingConstantRegion {
                ...
                try nextErrorMutex.withLock { error in if let error { throw error } }
            }
    ...
    func testViewOptimization() throws {
        ...
        // Test that view v is not included in the observed region. This optimization helps
        // observation of views that feed from a single table.
    ...
    func testPragmaTableOptimization() throws {
        ...
        // Test that no pragma table is included in the observed region. This optimization helps
        // observation that feed from a single table.
        let regionMutex: Mutex<DatabaseRegion?> = Mutex(nil)
        let expectation = self.expectation(description: "")
        let observation = ValueObservation
            .trackingConstantRegion { _ = try request.fetchAll($0) }
    ...
    func testTrackingExplicitRegion() throws {
        final class TestStream: TextOutputStream, Sendable {
            ...
            func write(_ string: String) { stringsMutex.withLock { $0.append(string) } }
        }
    ...
    func testDisallowedSnapshotOptimizationWithAsyncScheduler() throws {
        ...
        // Force DatabasePool to perform two initial fetches, because between its first read access,
        // and its write access that installs the transaction observer, some write did happen.
    ...
    func testAllowedSnapshotOptimizationWithAsyncScheduler() throws {
        ...
        // Allow pool to perform a single initial fetch, because between its first read access, and
        // its write access that installs the transaction observer, no write did happen.
        let needsChangeMutex = Mutex(true)
        let observation = ValueObservation.trackingConstantRegion { db -> Int in
            let needsChange = needsChangeMutex.withLock { needed in
                let wasNeeded = needed
    ...
    func test_ValueObservation_is_triggered_by_explicit_change_notification() throws {
        ...
        try dbQueue1.write { db in try db.execute(sql: "CREATE TABLE test(a)") }
        ...
        let observation = ValueObservation.tracking { db in try Table("test").fetchCount(db) }
        let cancellable = observation.start(
            in: dbQueue1,
            scheduling: .immediate,
            onError: { error in XCTFail("Unexpected error: \(error)") },
        ...
        try withExtendedLifetime(cancellable) {
            ...
            try dbQueue2.write { db in try db.execute(sql: "INSERT INTO test (a) VALUES (1)") }
            wait(for: [undetectedExpectation], timeout: 2)

            // ... until we perform an explicit change notification
            try dbQueue1.write { db in try db.notifyChanges(in: Table("test")) }
    ...
    func testCancellableLifetime() throws {
        ...
        cancellable = observation.start(
            ...
            onError: { error in XCTFail("Unexpected error: \(error)") },
            onChange: { _ in
                if changesCountMutex.increment() == 2 { cancellable = nil }
                notificationExpectation.fulfill()
            })

        // notified
        try dbQueue.write { db in try db.execute(sql: "INSERT INTO t DEFAULT VALUES") }
    ...
    func testCancellableExplicitCancellation() throws {
        ...
        cancellable = observation.start(
            ...
            onError: { error in XCTFail("Unexpected error: \(error)") },
            onChange: { _ in
                if changesCountMutex.increment() == 2 { cancellable.cancel() }
                notificationExpectation.fulfill()
            })
        ...
        func test(_ dbWriter: some DatabaseWriter) throws {
            ...
            do {
                ...
                cancellable = observation.start(
                    ...
                    scheduling: .immediate,
                    onError: { error in XCTFail("Unexpected error: \(error)") },
                    onChange: { _ in notificationExpectation.fulfill() })
            }

            try dbWriter.write { db in try db.execute(sql: "INSERT INTO t DEFAULT VALUES") }
            wait(for: [notificationExpectation], timeout: 2)
        }
    ...
    func testIssue1550() throws {
        func test(_ writer: some DatabaseWriter) throws {
            ...
            // Perform a write after cancellation, but before the observation could schedule the
            // removal of its transaction observer from the writer dispatch queue.
            ...
            // Wait until a second write could run, after the observation has removed its
            // transaction observer from the writer dispatch queue.
            let secondWriteExpectation = expectation(description: "cancelled")
            writer.asyncWriteWithoutTransaction { _ in secondWriteExpectation.fulfill() }
            wait(for: [secondWriteExpectation], timeout: 5)

            // We should not have been notified of the first write, because it was performed after
            // cancellation.
    ...
    @MainActor func test_mainActor_observation() throws {
        let dbQueue = try makeDatabaseQueue()
        try dbQueue.write { db in
            try db.create(table: "test") { t in t.autoIncrementedPrimaryKey("id") }
        }

        let observation = ValueObservation.tracking { try Table("test").fetchCount($0) }
        ...
        let cancellable = observation.start(
            ...
            onChange: {
                value = $0
                if value == 2 { expectation.fulfill() }
            })
        ...
        withExtendedLifetime(cancellable) { _ in wait(for: [expectation], timeout: 2) }
    }
    ...
    func testManyObservations() throws {
        ...
        func test(
            ...
            let initialValueExpectation = expectation(description: "initialValue")
            #if SQLITE_ENABLE_SNAPSHOT && !SQLITE_DISABLE_SNAPSHOT
            ...
            let secondValueExpectation = expectation(description: "secondValue")
            ...
            try withExtendedLifetime(cancellables) {
                ...
                try writer.write { try $0.execute(sql: "INSERT INTO t DEFAULT VALUES") }
    ...
    func testManyObservationsWithLongConcurrentWrite() throws {
        ...
        func test(
            ...
            let secondValueExpectation = expectation(description: "")
            secondValueExpectation.expectedFulfillmentCount = observationCount
    ...
    // Regression test for <https://github.com/groue/GRDB.swift/issues/1362> start(in:) resolves to
    // the main-actor overload here
    @MainActor
    func testIssue1362() throws {
        func test(_ writer: some DatabaseWriter) throws {
            ...
            // Start an observation and wait until it has installed its transaction observer.
            ...
            let cancellable = initialObservation.start(
                in: writer,
                // Immediate initial value so that the next value comes from the write access that
                // installs the transaction observer.
                scheduling: .immediate,
                onError: { error in XCTFail("Unexpected error: \(error)") },
                onChange: { count in
                    if count == 1 { installedExpectation.fulfill() }
                    if count == 2 { finalExpectation.fulfill() }
                })
            cancellables.append(cancellable)
            ...
            // Wait until all observations are waiting for the writer so that they can install their
    ...
    // Regression test for <https://github.com/groue/GRDB.swift/issues/1383> start(in:) resolves to
    // the main-actor overload here
    @MainActor
    ...
    // Regression test for <https://github.com/groue/GRDB.swift/issues/1500> start(in:) resolves to
    // the main-actor overload here
    @MainActor
    func testIssue1500() throws {
        let pool = try makeDatabasePool()

        try pool.read { db in _ = try db.tableExists("t") }

        try pool.write { db in try db.create(table: "t") { t in t.column("a") } }

        _ = ValueObservation
            .trackingConstantRegion { db in try db.tableExists("t") }
            .start(
                in: pool,
                scheduling: .immediate,
                onError: { error in XCTFail("Unexpected error \(error)") },
                onChange: { value in XCTAssertEqual(value, true) })
    }
    ...
    @MainActor func testIssue1746() async throws {
        ...
        class CancelObserver: TransactionObserver {
            ...
            func databaseDidCommit(_: Database) { task.cancel() }
        }

        // Register CancelObserver first, so no other observer could access the database before the
        // task is cancelled.
        dbQueue.add(transactionObserver: CancelObserver(task: task), extent: .databaseLifetime)

        // Start observing. We expect to see 0, then 1.
