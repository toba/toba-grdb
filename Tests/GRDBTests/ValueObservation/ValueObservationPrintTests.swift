...
class ValueObservationPrintTests: GRDBTestCase {
    ...
    func test_readonly_success_asynchronousScheduling() throws {
        ...
        try dbPool.write { db in try db.execute(sql: "CREATE TABLE player(id INTEGER PRIMARY KEY)")
        }
        ...
        func test(_ dbReader: some DatabaseReader) throws {
            ...
            withExtendedLifetime(cancellable) {
                ...
                XCTAssertEqual(logger.strings, ["start", "fetch", "value: nil"])
            }
        }
    ...
    func test_readonly_failure_asynchronousScheduling() throws {
        ...
        let dbPool = try makeDatabasePool(filename: "test")
        // workaround Xcode 14.1 compiler (???) bug: database is not in the WAL mode unless the pool
        // is used.
        ...
        func test(_ dbReader: some DatabaseReader) throws {
            ...
            withExtendedLifetime(cancellable) {
                wait(for: [expectation], timeout: 1)
                XCTAssertEqual(logger.strings, ["start", "fetch", "failure: TestError()"])
            }
        }
    ...
    func test_concurrent_success_asynchronousScheduling() throws {
        ...
        withExtendedLifetime(cancellable) {
            wait(for: [expectation], timeout: 1)
            // Order of events may not be stable, because the first "value: nil" is notified
            // concurrently (from the reduce queue) with the first "database did change" (from the
            // writer queue). That's why we test the sorted output.
            XCTAssertEqual(
    ...
    func test_prefix() throws {
        let dbQueue = try makeDatabaseQueue()
        try dbQueue.write { db in try db.execute(sql: "CREATE TABLE player(id INTEGER PRIMARY KEY)")
        }
    ...
    func test_handleEvents() throws {
        func waitFor<R: ValueReducer>(_ observation: ValueObservation<R>) throws {
            ...
            withExtendedLifetime(cancellable) { wait(for: [expectation], timeout: 1) }
        }
