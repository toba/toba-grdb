...
class SharedValueObservationTests: GRDBTestCase {
    ...
    func test_immediate_observationLifetime() throws {
        ...
        var sharedObservation: SharedValueObservation<Int>? = ValueObservation
            ...
            .print(to: log)
            .shared(in: dbQueue, scheduling: .immediate, extent: .observationLifetime)
        ...
        withExtendedLifetime(sharedObservation) { sharedObservation in
            do {
                ...
                XCTAssertEqual(
                    log.flush(), ["start", "fetch", "tracked region: player(*)", "value: 0"])
                ...
                let cancellable1 = sharedObservation!.start(
                    ...
                    onChange: { value in
                        ...
                        _ = sharedObservation!.start(
                            onError: { XCTFail("Unexpected error \($0)") },
                            onChange: { value in value2Mutex.store(value) })
                    })
    ...
    func test_immediate_whileObserved() throws {
        ...
        var sharedObservation: SharedValueObservation<Int>? = ValueObservation
            .tracking(Table("player").fetchCount)
            .print(to: log)
            .shared(in: dbQueue, scheduling: .immediate, extent: .whileObserved)
        XCTAssertEqual(log.flush(), [])
    ...
    func test_async_observationLifetime() throws {
        ...
        var sharedObservation: SharedValueObservation<Int>? = ValueObservation
            .tracking(Table("player").fetchCount)
            .print(to: log)
            .shared(in: dbQueue, scheduling: .async(onQueue: .main), extent: .observationLifetime)
        XCTAssertEqual(log.flush(), [])
    ...
    func test_async_whileObserved() throws {
        ...
        var sharedObservation: SharedValueObservation<Int>? = ValueObservation
            .tracking(Table("player").fetchCount)
            .print(to: log)
            .shared(in: dbQueue, scheduling: .async(onQueue: .main), extent: .whileObserved)
        XCTAssertEqual(log.flush(), [])
    ...
    func test_task_observationLifetime() throws {
        ...
        var sharedObservation: SharedValueObservation<Int>? = ValueObservation
            .tracking(Table("player").fetchCount)
            .print(to: log)
            .shared(in: dbQueue, scheduling: .task, extent: .observationLifetime)
        XCTAssertEqual(log.flush(), [])
    ...
    func test_task_whileObserved() throws {
        ...
        var sharedObservation: SharedValueObservation<Int>? = ValueObservation
            .tracking(Table("player").fetchCount)
            .print(to: log)
            .shared(in: dbQueue, scheduling: .task, extent: .whileObserved)
        XCTAssertEqual(log.flush(), [])
    ...
    func test_error_recovery_observationLifetime() throws {
        ...
        let publisher =
            ValueObservation
            .tracking { db -> Int in
                try fetchErrorMutex.withLock { error in if let error { throw error } }
        ...
        do {
            ...
            if case .finished = try wait(for: recorder1.completion, timeout: 1) {
                XCTFail("Expected error")
            }
            if case .finished = try wait(for: recorder2.completion, timeout: 1) {
                XCTFail("Expected error")
            }
            XCTAssertEqual(
                log.flush(),
        ...
        do {
            let recorder = publisher.record()
            if case .finished = try wait(for: recorder.completion, timeout: 1) {
                XCTFail("Expected error")
            }
            XCTAssertEqual(log.flush(), [])
        }
    ...
    func test_error_recovery_whileObserved() throws {
        ...
        do {
            ...
            try XCTAssertEqual(wait(for: recorder.next(), timeout: 1), 1)
            XCTAssertEqual(
                log.flush(), ["start", "fetch", "tracked region: player(*)", "value: 1"])
        }
    }
    #endif
...
private final class Log: TextOutputStream, Sendable {
    private let stringsMutex = Mutex<[String]>([])
