...
class ValueObservationPublisherTests: XCTestCase {
    ...
    func testDefaultSchedulerChangesNotifications() throws {
        ...
        func test(writer: some DatabaseWriter) throws {
            ...
            if writer is DatabaseQueue {
                ...
                // TODO: prefix(until:)
                let elements = try wait(
                    for: recorder.prefix(expectedElements.count + 2).inverted, timeout: 1)
    ...
    func testDefaultSchedulerError() throws {
        func test(writer: some DatabaseWriter) throws {
            ...
            switch completion {
                case let .failure(error): XCTAssertNotNil(error as? DatabaseError)
                case .finished: XCTFail("Expected error")
            }
        }
    ...
    func testDemandOneDoesNotReceiveTwoElements() throws {
        ...
        func test(writer: some DatabaseWriter) throws {
            ...
            ValueObservation
                .trackingConstantRegion(Player.fetchCount)
                .publisher(
                    in: writer, scheduling: .immediate /* make sure we get the initial db state */
                )
                .subscribe(subscriber)
    ...
    func testIssue1194() throws {
        ...
        migrator.registerMigration("v1") { db in
            try db.create(table: Record.databaseTableName) { t in t.autoIncrementedPrimaryKey("id")
            }
        }

        try migrator.migrate(database)

        let observation = ValueObservation.tracking { db in try Record.fetchCount(db) }

        let exp = expectation(description: "")
        let cancellable = observation.publisher(in: database, scheduling: .immediate)
            .map { _ in database.readPublisher { db in try Record.fetchCount(db) } }
            .switchToLatest()
            .sink(receiveCompletion: { _ in }, receiveValue: { _ in exp.fulfill() })

        withExtendedLifetime(cancellable) { wait(for: [exp], timeout: 1) }
    }
}
#endif
