...
class DatabaseWriterWritePublisherTests: XCTestCase {
    ...
    func testDeadlockPrevention() throws {
        ...
        func test(writer: some DatabaseWriter, iteration _: Int) throws {
            // print(iteration)
        ...
        try Test(repeatCount: 100, test).runAtTemporaryDatabasePath {
            try setUp(DatabaseQueue(path: $0))
        }
        try Test(repeatCount: 100, test).runAtTemporaryDatabasePath {
            try setUp(DatabasePool(path: $0))
        }
    }
}
#endif
