...
class DatabasePoolConcurrencyTests: GRDBTestCase {
    func testDatabasePoolFundamental1() throws {
        ...
        //
        // This test shows (if needed) that writes that are not wrapped in a transaction can not
        // provide our guarantee, and that deferred transaction provide the immutable view of the
        // database needed by dbPool.read().
        //
        // Reader Writer BEGIN DEFERRED TRANSACTION
        ...
        // the two blocks open connections in a fixed order, so both need the test case
        nonisolated(unsafe) let testCase = self
        let s0 = DispatchSemaphore(value: 0)
        let block1: @Sendable () -> Void = {
            let dbQueue = try! testCase.makeDatabaseQueue(filename: "test.sqlite")
            ...
            try! dbQueue.writeWithoutTransaction { db in
                ...
                try XCTAssertEqual(
                    Int.fetchOne(db, sql: "SELECT SUM(value) AS balance FROM moves"), 1)  // Non-zero balance
                s3.signal()
    ...
    func testDatabasePoolFundamental2() throws {
        // Constraint: the sum of values, the balance, must remain zero.
        //
        // This test shows that deferred transactions play well with concurrent immediate
        // transactions.
        //
        // Reader Writer BEGIN IMMEDIATE TRANSACTION
        let s1 = DispatchSemaphore(value: 0)
        ...
        // SELECT SUM(value) AS balance FROM moves END END
        ...
        let block1: @Sendable () -> Void = {
            ...
            try! dbQueue.writeWithoutTransaction { db in
                ...
                _ = s3.wait(timeout: .distantFuture)
                try XCTAssertEqual(
                    Int.fetchOne(db, sql: "SELECT SUM(value) AS balance FROM moves"), 0)  // Zero balance
                s4.signal()
    ...
    func testWrappedReadWrite() throws {
        ...
        let id = try dbPool.read { db in try Int.fetchOne(db, sql: "SELECT id FROM items")! }
        XCTAssertEqual(id, 1)
    }
    ...
    func testReadError() throws {
        ...
        // Block 1 Block 2 PRAGMA locking_mode=EXCLUSIVE CREATE TABLE
        // >
        let s1 = DispatchSemaphore(value: 0)
        // dbPool.read // throws
    ...
    func testConcurrentRead() throws {
        ...
        try dbPool.write { db in
            try db.execute(sql: "CREATE TABLE items (id INTEGER PRIMARY KEY)")
            for _ in 0..<3 { try db.execute(sql: "INSERT INTO items (id) VALUES (NULL)") }
        }
    ...
    func testReadMethodIsolationOfStatement() throws {
        ...
        let block2: @Sendable () -> Void = {
            do {
                _ = s1.wait(timeout: .distantFuture)
                defer { s2.signal() }
                try dbPool.writeWithoutTransaction { db in try db.execute(sql: "DELETE FROM items")
                }
            } catch {
                XCTFail("error: \(error)")
            }
    ...
    func testReadBlockIsolationStartingWithRead() throws {
        let dbPool = try makeDatabasePool()
        try dbPool.write { db in try db.execute(sql: "CREATE TABLE items (id INTEGER PRIMARY KEY)")
        }
        ...
        let block1: @Sendable () -> Void = {
            try! dbPool.read { db in
                s1.signal()
                _ = s2.wait(timeout: .distantFuture)
                // We read 0 due to snaphot isolation: the reader has checked the schema version
    ...
    func testReadBlockIsolationStartingWithWrite() throws {
        ...
        // Block 1 Block 2 INSERT INTO items (id) VALUES (NULL)
        // >
        let s1 = DispatchSemaphore(value: 0)
        //                              dbPool.read {
        ...
        let block2: @Sendable () -> Void = {
            try! dbPool.read { db in
                _ = s1.wait(timeout: .distantFuture)
                try XCTAssertEqual(Int.fetchOne(db, sql: "SELECT COUNT(*) FROM items")!, 1)
                s2.signal()
                _ = s3.wait(timeout: .distantFuture)
                try XCTAssertEqual(Int.fetchOne(db, sql: "SELECT COUNT(*) FROM items")!, 1)
                s4.signal()
    ...
    func testReadBlockIsolationStartingWithWriteTransaction() throws {
        ...
        // Block 1 Block 2 BEGIN IMMEDIATE TRANSACTION INSERT INTO items (id) VALUES (NULL)
        // >
        let s1 = DispatchSemaphore(value: 0)
        //                              dbPool.read {
        //                              SELECT COUNT(*) FROM items -> 0
    ...
    func testLongRunningReadTransaction() throws {
        ...
        try dbQueue.writeWithoutTransaction { db in try db.beginTransaction(.deferred) }

        try dbQueue.writeWithoutTransaction { db in
            try XCTAssertEqual(Int.fetchOne(db, sql: "SELECT COUNT(*) FROM t")!, 1)
        }

        try dbPool.write { db in try db.execute(sql: "INSERT INTO t DEFAULT VALUES") }
    ...
    func testIssue80() throws {
        ...
        //
        // Here we test that `SELECT * FROM search`, which is announced by SQLite has a statement
        // that modifies the sqlite_master table, is still allowed as a regular select statement.
        //
        // This test uses a database pool, a connection for creating the search virtual table, and
        // another connection for reading.
        //
        ...
        try dbPool.read { db in _ = try Row.fetchAll(db, sql: "SELECT * FROM search") }
    }
    ...
    func testDefaultLabel() throws {
        ...
        dbPool.writeWithoutTransaction { db in
            ...
            #if canImport(Darwin)
            // This test CAN break in future releases: the dispatch queue labels are documented to
            // be a debug-only tool.
        ...
        let block1: @Sendable () -> Void = {
            try! dbPool.read { db in
                ...
                #if canImport(Darwin)
                // This test CAN break in future releases: the dispatch queue labels are documented
                // to be a debug-only tool.
                let label = String(utf8String: __dispatch_queue_get_label(nil))
    ...
    func testTargetQueue() throws {
        func test(targetQueue: DispatchQueue) throws {
            ...
            let dbPool = try makeDatabasePool()
            try dbPool.write { _ in dispatchPrecondition(condition: .onQueue(targetQueue)) }
            try dbPool.read { _ in dispatchPrecondition(condition: .onQueue(targetQueue)) }
        }
        ...
        // the point of the test is to run the body off the main queue, so the closure has to reach
        // a local function that captures the test case
        nonisolated(unsafe) let runOnMainQueue = { try test(targetQueue: .main) }
    ...
    func testWriteTargetQueue() throws {
        func test(targetQueue: DispatchQueue, writeTargetQueue: DispatchQueue) throws {
            ...
            let dbPool = try makeDatabasePool()
            try dbPool.write { _ in dispatchPrecondition(condition: .onQueue(writeTargetQueue)) }
            try dbPool.read { _ in dispatchPrecondition(condition: .onQueue(targetQueue)) }
        }
        ...
        // the point of the test is to run the body off the main queue, so the closure has to reach
        // a local function that captures the test case
        nonisolated(unsafe) let runOnMainQueue = {
    ...
    func testQoS() throws {
        ...
        func test(qos: DispatchQoS) throws {
            ...
            //
            // > [...] the default target queue [for a serial queue] is the [default] overcommit
            //   [global concurrent] queue.
            //
            // We want this default target queue in order to test database QoS with
            // dispatchPrecondition(condition:).
            //
            // > [...] You can get a reference to the overcommit queue by dropping down to the C
            //   function dispatch_get_global_queue (available in Swift with a __ prefix) and
            //   passing the private value of DISPATCH_QUEUE_OVERCOMMIT.
            // >
            // > [...] Of course you should not do this in production code, because
            //   DISPATCH_QUEUE_OVERCOMMIT is not a public API. I don't know of a way to get a
            //   reference to the overcommit queue using only public APIs.
    ...
    func testAsyncConcurrentReadOpensATransaction() throws {
        ...
        dbPool.writeWithoutTransaction { _ in
    ...
    func testAsyncConcurrentReadOutsideOfTransaction() throws {
        let dbPool = try makeDatabasePool()
        try dbPool.write { db in
            try db.create(table: "persons") { t in t.primaryKey("id", .integer) }
        }
    ...
    func testBarrierLocksReads() throws {
        ...
        DispatchQueue.global().async {
            ...
            fetchedValueMutex.store(
                try! dbPool.read { db in try Int.fetchOne(db, sql: "SELECT id FROM items")! })
            expectation.fulfill()
            s3.signal()
        }
    ...
    func testConcurrentOpening() throws {
        ...
        for _ in 0..<50 {
            ...
            DispatchQueue.concurrentPerform(iterations: 10) { _ in
                // WTF I could never Google for the proper correct error handling of
                // NSFileCoordinator. What a weird API.
    ...
    private func openSharedDatabase(at databaseURL: URL) throws -> DatabasePool {
        ...
        if let error = dbError ?? coordinatorError { throw error }
        return dbPool!
        #endif
    }

    // Test for sample code in Documentation.docc/DatabaseSharing.md. This test passes if this
    // method compiles
    private func openDatabase(at databaseURL: URL) throws -> DatabasePool {
        try DatabasePool(path: databaseURL.path)
    }
