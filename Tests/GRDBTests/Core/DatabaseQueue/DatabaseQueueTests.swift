...
class DatabaseQueueTests: GRDBTestCase {
    ...
    func testInvalidFileFormat() throws {
        do {
            ...
            XCTAssert(["file is encrypted or is not a database", "file is not a database"].contains(
                error.message!))
        }
    }
    ...
    func testAddRemoveFunction() throws {
        ...
        let fn = DatabaseFunction("succ", argumentCount: 1) { dbValues in
            guard let int = Int.fromDatabaseValue(dbValues[0]) else { return nil }
            return int + 1
        }
        try dbQueue.inDatabase { db in
            db.add(function: fn)
            try XCTAssertEqual(Int.fetchOne(db, sql: "SELECT succ(1)"), 2)  // 2
    ...
    func testAddRemoveCollation() throws {
        ...
        let collation = DatabaseCollation("test_collation_foo") { string1, string2 in
            (string1 as NSString).localizedStandardCompare(string2)
        }
        try dbQueue.inDatabase { db in
    ...
    func testAllowsUnsafeTransactions() throws {
        ...
        try dbQueue.writeWithoutTransaction { db in try db.beginTransaction() }

        try dbQueue.writeWithoutTransaction { db in try db.commit() }
    }
    ...
    func testTargetQueue() throws {
        func test(targetQueue: DispatchQueue) throws {
            ...
            let dbQueue = try makeDatabaseQueue()
            try dbQueue.write { _ in dispatchPrecondition(condition: .onQueue(targetQueue)) }
            try dbQueue.read { _ in dispatchPrecondition(condition: .onQueue(targetQueue)) }
        }
    ...
    func testWriteTargetQueue() throws {
        func test(targetQueue: DispatchQueue, writeTargetQueue: DispatchQueue) throws {
            ...
            let dbQueue = try makeDatabaseQueue()
            try dbQueue.write { _ in dispatchPrecondition(condition: .onQueue(writeTargetQueue)) }
        ...
        throws
    {
        ...
        // DB1 DB2 BEGIN DEFERRED TRANSACTION READ
        ...
        throws
    {
        ...
        // WRITE
        let s2 = DispatchSemaphore(value: 0)
        // WRITE -> SQLITE_BUSY because write lock can't be acquired, even though DB2 does no longer
        // hold any lock.
    ...
    func test_busy_timeout_and_IMMEDIATE_transactions_do_prevent_SQLITE_BUSY() throws {
        ...
        // every iteration opens its own connection, so the closure needs the test case
        nonisolated(unsafe) let testCase = self
        DispatchQueue.concurrentPerform(iterations: parallelWritesCount) { [configuration] _ in
            let dbQueue = try! testCase.makeDatabaseQueue(
                filename: "test", configuration: configuration)
    ...
    func testClose() throws {
        ...
        do {
            try dbQueue.inDatabase { db in try db.execute(sql: "SELECT * FROM sqlite_master") }
            XCTFail("Expected Error")
    ...
    func testFailedClose() throws {
        ...
        XCTAssert(lastSQLiteDiagnostic!.message.contains(
