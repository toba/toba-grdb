...
class ForeignKeyInfoTests: GRDBTestCase {
    private func assertEqual(
        _ lhs: ForeignKeyInfo,
        _ rhs: ForeignKeyInfo,
    ...
    func testForeignKeys() throws {
        ...
        try dbQueue.inDatabase { db in
            ...
            try db.execute(
                sql: "CREATE TABLE children2 (parentId1 REFERENCES parents1, parentId2 REFERENCES parents1)"
            )
            try db.execute(
                sql: "CREATE TABLE children3 (parentA, parentB, FOREIGN KEY (parentA, parentB) REFERENCES parents2)"
            )
            try db.execute(
                sql: "CREATE TABLE children4 (parentA1, parentB1, parentA2, parentB2, FOREIGN KEY (parentA1, parentB1) REFERENCES parents2, FOREIGN KEY (parentA2, parentB2) REFERENCES parents2(b, a))"
            )
            ...
            do {
                ...
                assertEqual(
                    foreignKeys[0],
                    ForeignKeyInfo(
                        id: foreignKeys[0].id, destinationTable: "parents1",
                        mapping: [(origin: "parentId", destination: "id")]))
            }
            ...
            do {
                ...
                assertEqual(
                    ...
                    ForeignKeyInfo(
                        id: foreignKeys[0].id, destinationTable: "parents1",
                        mapping: [(origin: "parentId2", destination: "id")]))
                assertEqual(
                    foreignKeys[1],
                    ForeignKeyInfo(
                        id: foreignKeys[1].id, destinationTable: "parents1",
                        mapping: [(origin: "parentId1", destination: "id")]))
            }
            ...
            do {
                ...
                assertEqual(
                    foreignKeys[0],
                    ForeignKeyInfo(
                        id: foreignKeys[0].id, destinationTable: "parents2",
                        mapping: [
                            (origin: "parentA", destination: "a"),
                            (origin: "parentB", destination: "b"),
                        ]))
            }
            ...
            do {
                ...
                assertEqual(
                    ...
                    ForeignKeyInfo(
                        id: foreignKeys[0].id, destinationTable: "parents2",
                        mapping: [
                            (origin: "parentA2", destination: "b"),
                            (origin: "parentB2", destination: "a"),
                        ]))
                assertEqual(
                    foreignKeys[1],
                    ForeignKeyInfo(
                        id: foreignKeys[1].id, destinationTable: "parents2",
                        mapping: [
                            (origin: "parentA1", destination: "a"),
                            (origin: "parentB1", destination: "b"),
                        ]))
            }
        }
    }
    ...
    func testSpecifiedSchemaWithTableNameCollisions() throws {
        ...
        // Avoid error due to key not being provided: file is not a database - while executing
        // `ATTACH DATABASE...`
    ...
    // The `children` table in the attached database should not be found unless explicitly specified
    // as it is after `main.children` in resolution order.
    ...
    func testForeignKeyViolations() throws {
        try makeDatabaseQueue().writeWithoutTransaction { db in
            ...
            if let violation = violations.first(where: {
                $0.originTable == "child1" && $0.originRowID == 13
            }) {
            ...
            if let violation = violations.first(where: {
                $0.originTable == "child1" && $0.originRowID == 42
            }) {
                XCTAssertEqual(violation.destinationTable, "parent")
            } else {
                XCTFail("Missing violation")
            ...
            if let violation = violations.first(where: {
                $0.originTable == "child2" && $0.originRowID == 17
            }) {
                XCTAssertEqual(violation.destinationTable, "parent")
            } else {
                XCTFail("Missing violation")
    ...
    func testCheckForeignKeys() throws {
        try makeDatabaseQueue().writeWithoutTransaction { db in
            ...
            do {
                ...
                XCTAssertEqual(
                    error.message,
                    #"FOREIGN KEY constraint violation - from child(parentId) to parent(id), in [id:13 parentId:"1"]"#
                )
                XCTAssertEqual(
                    error.description,
                    #"SQLite error 19: FOREIGN KEY constraint violation - from child(parentId) to parent(id), in [id:13 parentId:"1"]"#
                )
            }
        }
        ...
        try makeDatabaseQueue().writeWithoutTransaction { db in
            ...
            do {
                ...
                XCTAssertEqual(
                    error.message,
                    #"FOREIGN KEY constraint violation - from child(parentA, parentB) to parent(a, b), in [id:13 parentA:"foo" parentB:"bar"]"#
                )
                XCTAssertEqual(
                    error.description,
                    #"SQLite error 19: FOREIGN KEY constraint violation - from child(parentA, parentB) to parent(a, b), in [id:13 parentA:"foo" parentB:"bar"]"#
                )
            }
        }
        ...
        try makeDatabaseQueue().writeWithoutTransaction { db in
            ...
            do {
                ...
                XCTAssertEqual(
                    error.message,
                    "FOREIGN KEY constraint violation - from child(parentA, parentB) to parent(a, b)"
                )
                XCTAssertEqual(
                    error.description,
                    "SQLite error 19: FOREIGN KEY constraint violation - from child(parentA, parentB) to parent(a, b)"
                )
            }
        }
    }
    ...
    // The `child` table in the attached database should not be found unless explicitly specified as
    // it is after `main.child` in resolution order.
