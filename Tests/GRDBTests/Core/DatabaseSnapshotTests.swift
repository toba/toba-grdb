...
class DatabaseSnapshotTests: GRDBTestCase {
    ...
    func testSnapshotCreatedFromTransactionObserver() throws {
        // Creating a snapshot from a didCommit callback is an important use case. But we know
        // SQLite snapshots created with sqlite3_snapshot_get() requires a transaction. This means
        // that creating a snapshot will open a transaction. We must make sure this transaction does
        // not create any deadlock of reentrancy issue with transaction observers.
        class Observer: TransactionObserver {
            ...
            func observes(eventsOfKind _: DatabaseEventKind) -> Bool { false }
            func databaseDidChange(with _: DatabaseEvent) {}
            func databaseDidCommit(_: Database) { snapshot = try! dbPool.makeSnapshot() }
    ...
    func testSnapshotIsReadOnly() throws {
        ...
        do {
            try snapshot.read { db in try db.execute(sql: "CREATE TABLE t(id INTEGER PRIMARY KEY") }
    ...
    func testSnapshotInheritPoolFunctions() throws {
        dbConfiguration.prepareDatabase { db in
            let function = DatabaseFunction("foo", argumentCount: 0, pure: true) { _ in "foo" }
    ...
    func testSnapshotInheritPoolCollations() throws {
        dbConfiguration.prepareDatabase { db in
            let collation = DatabaseCollation("reverse") { string1, string2 in
                (string1 == string2)
                    ? .orderedSame
                    : ((string1 < string2) ? .orderedDescending : .orderedAscending)
            }
        ...
        try snapshot.read { db in
            try XCTAssertEqual(
                String.fetchAll(db, sql: "SELECT text FROM items ORDER BY text COLLATE reverse"),
                ["c", "b", "a"])
        }
    }
    ...
    func testReadBlockIsolationStartingWithRead() throws {
        ...
        let block1: @Sendable () -> Void = {
            ...
            try! snapshot.read { db in
                ...
                // We read 0 due to snaphot isolation which was acquired before `s1` could let the
                // writer insert an item.
    ...
    func testAutomaticCheckpointDoesNotInvalidateSnapshot() throws {
        ...
        try dbPool.writeWithoutTransaction { db in
            ...
            for _ in 0..<1000 { try counter.increment(db) }
        }
        try XCTAssertEqual(snapshot.read(counter.value), 1)
    }
    ...
    func testSnapshotSchemaCache() throws {
        let dbPool = try makeDatabasePool()
        try dbPool.write { db in try db.execute(sql: "CREATE TABLE t(id INTEGER PRIMARY KEY)") }
        let snapshot = try dbPool.makeSnapshot()
        try snapshot.read { db in
    ...
    func testClose() throws {
        ...
        do {
            try snapshot.read { db in try db.execute(sql: "SELECT * FROM sqlite_master") }
            XCTFail("Expected Error")
    ...
    func testCloseWithCachedStatement() throws {
        let snapshot = try makeDatabasePool().makeSnapshot()
        try snapshot.read { db in _ = try db.cachedStatement(sql: "SELECT * FROM sqlite_master") }
        try snapshot.close()
