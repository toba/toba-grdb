...
class AssociationPrefetchingObservationTests: GRDBTestCase {
    private func assertRequestRegionEqual<T>(
        ...
        do {
            ...
            XCTAssertTrue(
                expectedDescriptions.contains(region.description), region.description, file: file,
                line: line)
        }
    ...
    func testIncludingAllHasManyIncludingAllHasMany() throws {
        ...
        try dbQueue.read { db in
            ...
            do {
                ...
                try assertRequestRegionEqual(
                    db, request, "a(cola1,cola2),c(colc1,colc2),d(cold1,cold2,cold3)")
            }
                ...
                let request = A
                    ...
                    .including(
                        all: A
                            .hasMany(
                                D.self,
                                through: A.hasMany(C.self).filter(Column("colc1") == 8).forKey(
                                    "cs1"), using: C.hasMany(D.self)
                            )
                            .forKey("ds1")
                    )
                    .including(
                        all: A
                            .hasMany(
                                D.self, through: A.hasMany(C.self).forKey("cs2"),
    ...
    func testIncludingOptionalBelongsToIncludingAllHasMany() throws {
        ...
        try dbQueue.write { db in
            ...
            do {
                ...
                try assertRequestRegionEqual(
                    db, request, "a(cola1,cola2),b(colb1,colb2,colb3),c(colc1,colc2)")
            }
        }
    }
            ...
            do {
                let request = D
                    .joining(
                        optional: D
                            .hasOne(
                                A.self, through: D.belongsTo(C.self), using: C.belongsTo(A.self)
                            )
                            .including(
                                all: A
                                    .hasMany(B.self)
