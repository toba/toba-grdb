...
class QueryInterfaceRequestTests: GRDBTestCase {
    ...
    override func setUp() {
        super.setUp()
        // capture the collation, not self. prepareDatabase takes a Sendable closure
        let collation = collation
        dbConfiguration.prepareDatabase { db in db.add(collation: collation) }
    }
    ...
    func testFetchRowFromRequest() throws {
        ...
        try dbQueue.inDatabase { db in
            try db.execute(
                sql: "INSERT INTO readers (name, age) VALUES (?, ?)", arguments: ["Arthur", 42])
            try db.execute(
                sql: "INSERT INTO readers (name, age) VALUES (?, ?)", arguments: ["Barbara", 36])
            ...
            do {
                ...
                while let row = try rows.next() { names.append(row["name"]) }
                XCTAssertEqual(lastSQLQuery, "SELECT * FROM \"readers\"")
    ...
    func testFetchCount() throws {
        let dbQueue = try makeDatabaseQueue()
        try dbQueue.inDatabase { db in
            try XCTAssertEqual(tableRequest.fetchCount(db), 0)
            XCTAssertEqual(lastSQLQuery, "SELECT COUNT(*) FROM \"readers\"")

            try XCTAssertEqual(tableRequest.reversed().fetchCount(db), 0)
            XCTAssertEqual(lastSQLQuery, "SELECT COUNT(*) FROM \"readers\"")

            try XCTAssertEqual(tableRequest.order(Columns.name).fetchCount(db), 0)
            XCTAssertEqual(lastSQLQuery, "SELECT COUNT(*) FROM \"readers\"")

            try XCTAssertEqual(tableRequest.order { $0.name }.fetchCount(db), 0)
            XCTAssertEqual(lastSQLQuery, "SELECT COUNT(*) FROM \"readers\"")

            try XCTAssertEqual(tableRequest.limit(10).fetchCount(db), 0)
            XCTAssertEqual(
                lastSQLQuery, "SELECT COUNT(*) FROM (SELECT * FROM \"readers\" LIMIT 10)")

            try XCTAssertEqual(tableRequest.filter(Columns.age == 42).fetchCount(db), 0)
            XCTAssertEqual(lastSQLQuery, "SELECT COUNT(*) FROM \"readers\" WHERE \"age\" = 42")

            try XCTAssertEqual(tableRequest.filter { $0.age == 42 }.fetchCount(db), 0)
            XCTAssertEqual(lastSQLQuery, "SELECT COUNT(*) FROM \"readers\" WHERE \"age\" = 42")

            try XCTAssertEqual(tableRequest.distinct().fetchCount(db), 0)
            XCTAssertEqual(
                lastSQLQuery, "SELECT COUNT(*) FROM (SELECT DISTINCT * FROM \"readers\")")

            try XCTAssertEqual(tableRequest.select(Columns.name).fetchCount(db), 0)
            XCTAssertEqual(lastSQLQuery, "SELECT COUNT(*) FROM \"readers\"")

            try XCTAssertEqual(tableRequest.select { $0.name }.fetchCount(db), 0)
            XCTAssertEqual(lastSQLQuery, "SELECT COUNT(*) FROM \"readers\"")

            try XCTAssertEqual(tableRequest.select(Columns.name).distinct().fetchCount(db), 0)
            XCTAssertEqual(lastSQLQuery, "SELECT COUNT(DISTINCT \"name\") FROM \"readers\"")

            try XCTAssertEqual(tableRequest.select { $0.name }.distinct().fetchCount(db), 0)
            XCTAssertEqual(lastSQLQuery, "SELECT COUNT(DISTINCT \"name\") FROM \"readers\"")

            try XCTAssertEqual(tableRequest.select(Columns.age * 2).distinct().fetchCount(db), 0)
            XCTAssertEqual(lastSQLQuery, "SELECT COUNT(DISTINCT \"age\" * 2) FROM \"readers\"")

            try XCTAssertEqual(tableRequest.select { $0.age * 2 }.distinct().fetchCount(db), 0)
            XCTAssertEqual(lastSQLQuery, "SELECT COUNT(DISTINCT \"age\" * 2) FROM \"readers\"")

            try XCTAssertEqual(
                tableRequest.select((Columns.age * 2).forKey("ignored")).distinct().fetchCount(db),
                0)
            XCTAssertEqual(lastSQLQuery, "SELECT COUNT(DISTINCT \"age\" * 2) FROM \"readers\"")

            try XCTAssertEqual(
                tableRequest.select { ($0.age * 2).forKey("ignored") }.distinct().fetchCount(db), 0)
            XCTAssertEqual(lastSQLQuery, "SELECT COUNT(DISTINCT \"age\" * 2) FROM \"readers\"")

            try XCTAssertEqual(tableRequest.select(Columns.name, Columns.age).fetchCount(db), 0)
            XCTAssertEqual(lastSQLQuery, "SELECT COUNT(*) FROM \"readers\"")

            try XCTAssertEqual(tableRequest.select { [$0.name, $0.age] }.fetchCount(db), 0)
            XCTAssertEqual(lastSQLQuery, "SELECT COUNT(*) FROM \"readers\"")

            try XCTAssertEqual(
                tableRequest.select(Columns.name, Columns.age).distinct().fetchCount(db), 0)
            XCTAssertEqual(
                lastSQLQuery,
                "SELECT COUNT(*) FROM (SELECT DISTINCT \"name\", \"age\" FROM \"readers\")")

            try XCTAssertEqual(
                tableRequest.select { [$0.name, $0.age] }.distinct().fetchCount(db), 0)
            XCTAssertEqual(
                lastSQLQuery,
                "SELECT COUNT(*) FROM (SELECT DISTINCT \"name\", \"age\" FROM \"readers\")")

            try XCTAssertEqual(
                tableRequest.select(max(Columns.age)).group(Columns.name).fetchCount(db), 0)
            XCTAssertEqual(
                lastSQLQuery,
                "SELECT COUNT(*) FROM (SELECT MAX(\"age\") FROM \"readers\" GROUP BY \"name\")")

            try XCTAssertEqual(
                tableRequest.select { max($0.age) }.group { $0.name }.fetchCount(db), 0)
            XCTAssertEqual(
                lastSQLQuery,
                "SELECT COUNT(*) FROM (SELECT MAX(\"age\") FROM \"readers\" GROUP BY \"name\")")
        }
    ...
    func testIssue1357() throws {
        ...
        try dbQueue.inDatabase { db in
            ...
            try XCTAssertEqual(request.fetchCount(db), 0)
            XCTAssertEqual(
                lastSQLQuery,
                """
    ...
    func testSelectSQLLiteral() throws {
        ...
        try dbQueue.inDatabase { db in
            ...
            try test(tableRequest.select(literal: SQL(
                sql: ":name, id - :value", arguments: ["name": "O'Brien", "value": 1])))
            // Interpolation
    ...
    func testSelectionCustomKey() throws {
        ...
        try dbQueue.inDatabase { db in
            ...
            do {
                let request = tableRequest.select(
                    Columns.name.forKey("nom"), (Columns.age + 1).forKey("agePlusOne"))
                let row = try Row.fetchOne(db, request)!
                XCTAssertEqual(
                    lastSQLQuery,
                    "SELECT \"name\" AS \"nom\", \"age\" + 1 AS \"agePlusOne\" FROM \"readers\" LIMIT 1"
                )
            ...
            do {
                let request = tableRequest.select {
                    [$0.name.forKey("nom"), ($0.age + 1).forKey("agePlusOne")]
                }
                let row = try Row.fetchOne(db, request)!
                XCTAssertEqual(
                    lastSQLQuery,
    ...
    func testAnnotated() throws {
        ...
        try dbQueue.inDatabase { db in
            ...
            do {
                let request = tableRequest.select(Columns.name).annotated(
                    with: Columns.id - 1, Columns.id + 1)
                _ = try Row.fetchAll(db, request)
                XCTAssertEqual(
                    lastSQLQuery, "SELECT \"name\", \"id\" - 1, \"id\" + 1 FROM \"readers\"")
            }
            do {
        ...
        try dbQueue.inDatabase { db in
            ...
            do {
                ...
                while let row = try rows.next() {
                    // Just some sanity checks that the "author"."name" SQL column is simply exposed
                    // as "name" in Swift code:
            ...
            do {
                ...
                do {
                    ...
                    do {
                        let value = try Reader
                            .select(
                                literal: SQL(sql: "? AS name", arguments: ["O'Brien"]),
                                as: String.self
                            )
                            .fetchOne(db)!
            ...
            do {
                ...
                do {
                    ...
                    do {
                        let value = try Reader
                            .select(
                                literal: SQL(sql: "name, :age AS age", arguments: ["age": 22]),
                                as: Row.self
                            )
                            .fetchOne(db)!
                ...
                do {
                    ...
                    do {
                        let value = try Reader
                            .all()
                            .select(
                                literal: SQL(sql: "name, ? AS age", arguments: [22]), as: Row.self
                            )
                            .fetchOne(db)!
                        XCTAssertEqual(value, ["name": "Arthur", "age": 22])
                    }
    ...
    func testSelectAsTypeInference() {
        ...
        func makeRequest1() -> QueryInterfaceRequest<String> { Reader.select(Columns.name) }

        func makeRequest3() -> QueryInterfaceRequest<String> { Reader.select { $0.name } }
    ...
    func testFilterLiteralWithPositionalArguments() throws {
        ...
        XCTAssertEqual(
            sql(
                dbQueue,
                tableRequest.filter(sql: "id <> ?", arguments: [1]).filter(true.databaseValue)),
            "SELECT * FROM \"readers\" WHERE (id <> 1) AND 1")
    }
    ...
    func testFilterLiteralWithNamedArguments() throws {
        ...
        XCTAssertEqual(
            sql(
                dbQueue,
                tableRequest.filter(sql: "id <> :id", arguments: ["id": 1]).filter(
                    true.databaseValue)),
            "SELECT * FROM \"readers\" WHERE (id <> 1) AND 1")
    }
    ...
    func testFilterOnView() throws {
        ...
        try dbQueue.write { db in
            ...
            XCTAssertEqual(lastSQLQuery, "SELECT * FROM \"v\" WHERE \"id\" = 1 LIMIT 1")
        }
    }
    ...
    func testHavingLiteralWithPositionalArguments() throws {
        ...
        XCTAssertEqual(
            sql(
                dbQueue,
                tableRequest.group(Columns.name).having(sql: "min(age) > ?", arguments: [18])),
            "SELECT * FROM \"readers\" GROUP BY \"name\" HAVING min(age) > 18")
    }
    ...
    func testHavingLiteralWithNamedArguments() throws {
        ...
        XCTAssertEqual(
            sql(
                dbQueue,
                tableRequest.group(Columns.name).having(
                    sql: "min(age) > :age", arguments: ["age": 18])),
            "SELECT * FROM \"readers\" GROUP BY \"name\" HAVING min(age) > 18")
    }
    ...
    func testMultipleHaving() throws {
        ...
        XCTAssertEqual(
            sql(
                dbQueue,
                tableRequest.group(Columns.name).having(min(Columns.age) > 18).having(
                    max(Columns.age) < 50)),
        ...
        XCTAssertEqual(
            sql(
                dbQueue,
                tableRequest.group { $0.name }.having { min($0.age) > 18 }.having {
                    max($0.age) < 50
                }),
            "SELECT * FROM \"readers\" GROUP BY \"name\" HAVING (MIN(\"age\") > 18) AND (MAX(\"age\") < 50)"
        )
    ...
    func testSort() throws {
        ...
        if Database.sqliteLibVersionNumber >= 3_030_000 {
            XCTAssertEqual(
    ...
    func testReverse() throws {
        let dbQueue = try makeDatabaseQueue()
        XCTAssertEqual(sql(dbQueue, tableRequest.reversed()), "SELECT * FROM \"readers\"")
        XCTAssertEqual(
    ...
    func testReverseWithCollation() throws {
        ...
        if Database.sqliteLibVersionNumber >= 3_030_000 {
            XCTAssertEqual(
                sql(
                    dbQueue,
                    tableRequest.order(Columns.name.collating(.nocase).ascNullsLast).reversed()),
                "SELECT * FROM \"readers\" ORDER BY \"name\" COLLATE NOCASE DESC NULLS FIRST")
            XCTAssertEqual(
                sql(
                    dbQueue,
                    tableRequest.order { $0.name.collating(.nocase).ascNullsLast }.reversed()),
                "SELECT * FROM \"readers\" ORDER BY \"name\" COLLATE NOCASE DESC NULLS FIRST")
            XCTAssertEqual(
                sql(
                    dbQueue,
                    tableRequest.order(Columns.name.collating(.nocase).descNullsFirst).reversed()),
                "SELECT * FROM \"readers\" ORDER BY \"name\" COLLATE NOCASE ASC NULLS LAST")
            XCTAssertEqual(
                sql(
                    dbQueue,
                    tableRequest.order { $0.name.collating(.nocase).descNullsFirst }.reversed()),
                "SELECT * FROM \"readers\" ORDER BY \"name\" COLLATE NOCASE ASC NULLS LAST")
        }
        #else
        if #available(iOS 14, macOS 10.16, tvOS 14, *) {
    ...
    func testLimit() throws {
        let dbQueue = try makeDatabaseQueue()
        XCTAssertEqual(sql(dbQueue, tableRequest.limit(1)), "SELECT * FROM \"readers\" LIMIT 1")
        XCTAssertEqual(
    ...
    func testFetchOneLimitOptimization() throws {
        ...
        try dbQueue.read { db in
            ...
            do {
                ...
                XCTAssertEqual(
                    lastSQLQuery, "SELECT * FROM \"readers\" WHERE \"name\" = \'Arthur\' LIMIT 1")
            }
            do {
                _ = try Row.fetchOne(
                    db, tableRequest.filter(Column("id") == 1 && Column("name") == "Arthur"))
                XCTAssertEqual(
                    lastSQLQuery,
                    "SELECT * FROM \"readers\" WHERE (\"id\" = 1) AND (\"name\" = \'Arthur\')")
            }
            do {
                _ = try Row.fetchOne(
                    db, tableRequest.filter(Column("id") == 1).filter(Column("name") == "Arthur"))
                XCTAssertEqual(
                    lastSQLQuery,
                    "SELECT * FROM \"readers\" WHERE (\"id\" = 1) AND (\"name\" = \'Arthur\')")
            }
            ...
            do {
                _ = try Row.fetchOne(
                    db, tableRequest.filter(Column("id") == 1 && Column("id") == 2))
                XCTAssertEqual(
                    lastSQLQuery, "SELECT * FROM \"readers\" WHERE (\"id\" = 1) AND (\"id\" = 2)")
            }
            do {
                _ = try Row.fetchOne(
                    db, tableRequest.filter(Column("id") == 1 || Column("id") == 2))
                XCTAssertEqual(
                    lastSQLQuery,
                    "SELECT * FROM \"readers\" WHERE (\"id\" = 1) OR (\"id\" = 2) LIMIT 1")
            }
            do {
            ...
            do {
                _ = try Row.fetchOne(
                    db,
                    tableRequest.select(
                        min(Column("age")).forKey("minAge"), max(Column("age")).forKey("maxAge")))
                XCTAssertEqual(
                    lastSQLQuery,
                    "SELECT MIN(\"age\") AS \"minAge\", MAX(\"age\") AS \"maxAge\" FROM \"readers\""
                )
            }
            do {
    ...
    func testIsEmpty() throws {
        ...
        try dbQueue.write { db in
            ...
            XCTAssertEqual(
                lastSQLQuery, "SELECT EXISTS (SELECT * FROM \"readers\" WHERE \"name\" = 'Arthur')")
        }
    }
}
