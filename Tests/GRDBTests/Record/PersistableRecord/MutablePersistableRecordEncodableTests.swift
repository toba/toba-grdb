...
extension MutablePersistableRecordEncodableTests {
    func testTrivialEncodable() throws {
        ...
        try dbQueue.inDatabase { db in
            try db.create(table: "t1") { t in t.column("value", .text) }
    ...
    func testCustomEncodable() throws {
        ...
        let dbQueue = try makeDatabaseQueue()
        try dbQueue.inDatabase { db in
            try db.create(table: "t1") { t in t.column("someColumn", .text) }
    ...
    // Regression test for <https://github.com/groue/GRDB.swift/issues/1565> Here we test that
    // `EncodableRecord` takes precedence over `Encodable` when a record is encoded with a
...
extension MutablePersistableRecordEncodableTests {
    ...
    func testNonTrivialSingleValueEncodableProperty() throws {
        ...
        try dbQueue.inTransaction { db in
            var value = Struct(
                value: Value(nestedValue: NestedValue(string: "foo")),
                optionalValue: Value(nestedValue: NestedValue(string: "bar")))
            try assert(value, isEncodedIn: ["value": "foo", "optionalValue": "bar"])
        ...
        try dbQueue.inTransaction { db in
            var value = Struct(
                value: Value(nestedValue: NestedValue(string: "foo")), optionalValue: nil)
            try assert(value, isEncodedIn: ["value": "foo", "optionalValue": nil])
    ...
    func testEncodableRawRepresentableProperty() throws {
        // This test is somewhat redundant with testSingleValueEncodableProperty, since a
        // RawRepresentable enum is a "single-value" Encodable.
        //
        // But with an explicit test for enums, we are sure that enums, widely used, are supported.
        enum Value: String, Encodable { case foo, bar }
    ...
    func testDatabaseValueConvertibleProperty() throws {
        ...
        //
        // Without this preference for databaseValue over encode(to:), Date would encode as a
        // double.
        struct Value: Encodable, DatabaseValueConvertible {
            ...
            static func fromDatabaseValue(_: DatabaseValue) -> Value? { fatalError("irrelevant") }
        }
        ...
        try dbQueue.inTransaction { db in
            ...
            try assert(
                value,
                isEncodedIn: [
                    "value": "foo (DatabaseValueConvertible)",
                    "optionalValue": "bar (DatabaseValueConvertible)",
                ])
        ...
        try dbQueue.inTransaction { db in
            ...
            try assert(
                value,
                isEncodedIn: ["value": "foo (DatabaseValueConvertible)", "optionalValue": nil])
...
extension MutablePersistableRecordEncodableTests {
    func testStructWithDate() throws {
        ...
        let dbQueue = try makeDatabaseQueue()
        try dbQueue.inDatabase { db in
            try db.create(table: "t1") { t in t.column("date", .datetime) }
            ...
            // Date has a default Encodable implementation which encodes a Double. We expect here a
            // String, because DatabaseValueConvertible has precedence over Encodable.
    ...
    func testStructWithURL() throws {
        ...
        let dbQueue = try makeDatabaseQueue()
        try dbQueue.inDatabase { db in
            try db.create(table: "t1") { t in t.column("url", .text) }
    ...
    func testStructWithUUID() throws {
        ...
        let dbQueue = try makeDatabaseQueue()
        try dbQueue.inDatabase { db in
            try db.create(table: "t1") { t in t.column("uuid", .blob) }
    ...
    func testJSONDataEncodingStrategy() throws {
        ...
        try dbQueue.inDatabase { db in
            ...
            let record = Record(
                data: data, optionalData: data, datas: [data], optionalDatas: [nil, data])
    ...
    func testJSONDateEncodingStrategy() throws {
        ...
        try dbQueue.inDatabase { db in
            ...
            let record = Record(
                date: date, optionalDate: date, dates: [date], optionalDates: [nil, date])
            try record.insert(db)
...
extension MutablePersistableRecordEncodableTests {
    struct NestedKeyed: Encodable {
        ...
        func encode(to encoder: Encoder) throws {
            ...
            try container.encodeIfPresent(
                encoder.userInfo[testKeyNested] as? String, forKey: .context)
        }
    }
    ...
    struct Record: Encodable, MutablePersistableRecord {
        ...
        func encode(to encoder: Encoder) throws {
            ...
            try container.encodeIfPresent(encoder.codingPath.last?.stringValue, forKey: .key)
            try container.encodeIfPresent(
                encoder.userInfo[testKeyRoot] as? String, forKey: .context)
        }
    }
    ...
    struct CustomizedRecord: Encodable, MutablePersistableRecord {
        ...
        static var databaseEncodingUserInfo: [CodingUserInfoKey: any Sendable] {
            [testKeyRoot: "GRDB root", testKeyNested: "GRDB nested"]
        }

        static func databaseJSONEncoder(for column: String) -> JSONEncoder {
            let encoder = JSONEncoder()
            encoder.outputFormatting = .sortedKeys
            encoder.userInfo = [testKeyRoot: "JSON root", testKeyNested: "JSON nested: \(column)"]
            return encoder
        }
