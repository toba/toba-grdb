...
extension FetchableRecordDecodableTests {
    ...
    func testCustomDecodable() throws {
        struct Struct: FetchableRecord, Decodable {
            ...
            private enum CodingKeys: String, CodingKey { case value = "someColumn" }
    ...
    func testCustomFetchableRecord() {
        struct Struct: FetchableRecord, Decodable {
            ...
            init(row: Row) { value = (row["value"] as String) + " (FetchableRecord)" }
        }
...
extension FetchableRecordDecodableTests {
    ...
    func testSingleValueDataProperty() throws {
        ...
        struct Struct: FetchableRecord, Decodable {
            static func databaseDataDecodingStrategy(
                ...
                column == "value" ? .custom { _ in Data([1, 2, 3]) } : .deferredToData
            }
            let value: Value
            let optionalValue: Value?
        }
    ...
    func testSingleValueDateProperty() throws {
        ...
        struct Struct: FetchableRecord, Decodable {
            static func databaseDateDecodingStrategy(
                ...
                column == "value"
                    ? .custom { _ in Date(timeIntervalSince1970: 0) } : .deferredToDate
            }
            let value: Value
            let optionalValue: Value?
        }
    ...
    func testDecodableRawRepresentableProperty() throws {
        // This test is somewhat redundant with testSingleValueDecodableProperty, since a
        // RawRepresentable enum is a "single-value" Decodable.
        //
        // But with an explicit test for enums, we are sure that enums, widely used, are supported.
        enum Value: String, Decodable { case foo, bar }
    ...
    func testDatabaseValueConvertibleProperty() throws {
        ...
        //
        // Without this preference for fromDatabaseValue(_:) over init(from:Decoder), Date would
        // only decode from doubles.
        struct Value: Decodable, DatabaseValueConvertible {
            ...
            static func fromDatabaseValue(_ databaseValue: DatabaseValue) -> Value? {
                if let string = String.fromDatabaseValue(databaseValue) {
                    Value(string: string + " (DatabaseValueConvertible)")
                } else {
                    nil
                }
            }
...
extension FetchableRecordDecodableTests {
    func testStructWithData() throws {
        ...
        do {
            ...
            do {
                try dbQueue.read { _ = try StructWithData.fetchOne($0, sql: "SELECT NULL AS data") }
                XCTFail("Expected Error")
            } catch let error as RowDecodingError {
                switch error.impl {
                    case .valueMismatch:
    ...
    func testStructWithDate() throws {
        ...
        do {
            ...
            do {
                try dbQueue.read { _ = try StructWithDate.fetchOne($0, sql: "SELECT NULL AS date") }
                XCTFail("Expected Error")
            } catch let error as RowDecodingError {
                switch error.impl {
                    case .valueMismatch:
...
extension FetchableRecordDecodableTests {
    func testOptionalNestedStruct() throws {
        ...
        try dbQueue.inDatabase { db in
            try db.create(table: "t1") { t in t.column("nested", .text) }
    ...
    func testOptionalNestedArrayStruct() throws {
        ...
        try dbQueue.inDatabase { db in
            ...
            guard let arrayOfNestedModel = parentModel.first?.nested,
                  let firstNestedModelInArray = arrayOfNestedModel.first
            else {
                XCTFail()
                return
            }
    ...
    func testNonOptionalNestedArrayStruct() throws {
        ...
        let dbQueue = try makeDatabaseQueue()
        try dbQueue.inDatabase { db in try db.create(table: "t1") { t in t.column("nested", .text) }
        }
    ...
    func testCodableExampleCode() throws {
        ...
        try dbQueue.inDatabase { db in
            ...
            let value = Player(
                name: "PlayerName", score: 10, scores: [1, 2, 3, 4, 5], lastMedal: medal1,
                medals: [medal1, medal2], timeline: timeline)
            try value.insert(db)

            let parentModel = try Player.fetchAll(db)

            guard let first = parentModel.first,
                  let firstNestedModelInArray = first.medals.first,
                  let secondNestedModelInArray = first.medals.last
            else {
                XCTFail()
                return
            }
    ...
    func testArrayOfDetachedRowsAsData() throws {
        ...
        let dbQueue = try makeDatabaseQueue()
        try dbQueue.inDatabase { db in
            try db.create(table: "t1") { t in t.column("name", .text) }
        ...
        try dbQueue.read { db in
            ...
            for row in array {
                ...
                try row.withUnsafeData(named: "name") { data in XCTAssertEqual(jsonAsData, data) }
            }
        }
    }
    ...
    func testJSONDataEncodingStrategy() throws {
        ...
        try dbQueue.inDatabase { db in
            let data = "foo".data(using: .utf8)!
            let record = try Record.fetchOne(
                db, sql: "SELECT ? AS data, ? AS optionalData, ? AS datas, ? AS optionalDatas",
                arguments: [
                    data,
                    data,
                    "[\"Zm9v\"]",
    ...
    func testJSONDateEncodingStrategy() throws {
        ...
        try dbQueue.inDatabase { db in
            let record = try Record.fetchOne(
                db, sql: "SELECT ? AS date, ? AS optionalDate, ? AS dates, ? AS optionalDates",
                arguments: [
    ...
    func testDatabaseColumnDecodingStrategy_useDefaultKeys() throws {
        ...
        do {
            let record = try FetchableRecordDecoder().decode(
                Record.self,
                from: [
                    "requiredId": 1,
                    "optionalName": "test1",
        ...
        do {
            _ = try FetchableRecordDecoder().decode(
                Record.self,
                from: [
                    "required_id": 1,
                    "optionalName": "test1",
    ...
    func testDatabaseColumnDecodingStrategy_custom() throws {
        ...
        struct Record: FetchableRecord, Decodable, Equatable {
            static let databaseColumnDecodingStrategy = DatabaseColumnDecodingStrategy.custom {
                column in AnyKey(stringValue: String(column.dropFirst()))
            }
            let requiredId: Int
            let optionalName: String?
            let requiredDates: [Date]
...
extension FetchableRecordDecodableTests {
    ...
    struct Record: Decodable, FetchableRecord {
        ...
        enum CodingKeys: String, CodingKey { case nestedKeyed, nestedSingle, nestedUnkeyed }
    ...
    class CustomizedRecord: Decodable, FetchableRecord {
        ...
        static var databaseDecodingUserInfo: [CodingUserInfoKey: any Sendable] {
            [testKeyRoot: "GRDB root", testKeyNested: "GRDB column or scope"]
        }
    ...
    func testRecord1() throws {
        ...
        try dbQueue.read { db in
            ...
            let adapter = SuffixRowAdapter(fromIndex: 1).addingScopes([
                "nestedKeyed": RangeRowAdapter(0..<1)
            ])
    ...
    func testMissingKeys1() throws {
        ...
        // No error expected:
        // - a is successfully decoded because it consumes the one and unique allowed missing key
        // - b and c are successfully decoded, because they are optionals, and all optionals decode
        //   missing keys are nil. This is because GRDB records accept rows with missing columns,
        //   and b and c may want to decode columns.
        ...
        try makeDatabaseQueue().read { db in
            do {
                ...
                let row = try Row.fetchOne(
                    db, sql: "SELECT NULL", adapter: ScopeAdapter(["a": EmptyRowAdapter()]))!
            ...
            do {
                ...
                let row = try Row.fetchOne(
                    db, sql: "SELECT NULL", adapter: ScopeAdapter(["b": EmptyRowAdapter()]))!
                _ = try FetchableRecordDecoder().decode(Composed.self, from: row)
                XCTFail("Expected error")
            } catch let DecodingError.keyNotFound(key, context) {
            ...
            do {
                ...
                let row = try Row.fetchOne(
                    db, sql: "SELECT NULL", adapter: ScopeAdapter(["c": EmptyRowAdapter()]))!
                _ = try FetchableRecordDecoder().decode(Composed.self, from: row)
                XCTFail("Expected error")
            } catch let DecodingError.keyNotFound(key, context) {
...
extension FetchableRecordDecodableTests {
        ...
        struct Witness: Decodable, FetchableRecord {
            init(from decoder: any Decoder) throws {
                ...
                do {
                    ...
                    // Test contains(_:)
                    for key in allKeys { XCTAssertTrue(container.contains(key)) }
                ...
                do {
                    ...
                    XCTAssertEqual(Set(allKeys.map(\.stringValue)), ["c"])

                    // Test contains(_:)
                    for key in allKeys { XCTAssertTrue(topLevelScope1Container.contains(key)) }
                }
                ...
                do {
                    ...
                    // Test contains(_:)
                    for key in allKeys { XCTAssertTrue(topLevelScope2Container.contains(key)) }
                }
            }
        }
            ...
            do {
                let row = try Row.fetchOne(
                    ...
                    sql: """
                        ...
                        """,
                    adapter: ScopeAdapter(["nested": RangeRowAdapter(1..<2)]))!
            ...
            do {
                let row = try Row.fetchOne(
                    ...
                    sql: """
                        SELECT 1 AS a
                        """,
                    adapter: ScopeAdapter(["a": SuffixRowAdapter(fromIndex: 0)]))!
    ...
    func testSingleValueContainer() throws {
        ...
        struct Wrapper<Model: Decodable>: FetchableRecord, Decodable {
            ...
            enum CodingKeys: String, CodingKey { case otherValue }
    ...
    // Regression test for <https://github.com/groue/GRDB.swift/issues/1572> Here we test that
    // `FetchableRecord` takes precedence over `Decodable` when a record is encoded with a
    // `SingleValueEncodingContainer`.
    func testSingleValueContainerWithFetchableRecord() throws {
        struct Struct: Decodable, FetchableRecord {
            let value: String

            init(row: Row) throws { value = row["actualValue"] }
        }
