...
class DatabaseMigratorTests: GRDBTestCase {
    ...
    func testNonEmptyMigratorSync() throws {
        ...
        func test(writer: some DatabaseWriter) throws {
            ...
            try writer.read { db in
                try XCTAssertTrue(db.tableExists("persons"))
                try XCTAssertTrue(db.tableExists("pets"))
            }

            try migrator2.migrate(writer)
            try writer.read { db in
                try XCTAssertTrue(db.tableExists("persons"))
                try XCTAssertFalse(db.tableExists("pets"))
            }
        }
    ...
    func testMigrateUpTo() throws {
        ...
        func test(writer: some DatabaseWriter) throws {
            ...
            try writer.read { db in
                try XCTAssertTrue(db.tableExists("a"))
                try XCTAssertFalse(db.tableExists("b"))
            }
            ...
            try writer.read { db in
                try XCTAssertTrue(db.tableExists("a"))
                try XCTAssertTrue(db.tableExists("b"))
                try XCTAssertTrue(db.tableExists("c"))
            }

            // zero step
            try migrator.migrate(writer, upTo: "c")
            try migrator.migrate(writer)
            // fatal error: undefined migration: "missing" try migrator.migrate(writer, upTo:
            // "missing") fatal error: database is already migrated beyond migration "b" try
            // migrator.migrate(writer, upTo: "b")
        }
    ...
    func testMigrationFailureTriggersRollback() throws {
        ...
        migrator.registerMigration("createPersons") { db in
            ...
            try db.execute(
                sql: "CREATE TABLE pets (masterId INTEGER NOT NULL REFERENCES persons(id), name TEXT)"
            )
        ...
        do {
            ...
            do {
                ...
                // The first migration should be committed. The second migration should be
                // rollbacked.
    ...
    func testForeignKeyViolation() throws {
        ...
        migrator.registerMigration("removePersonTmpColumn") { db in
            // Test the technique described at
        ...
        do {
            ...
            // Migration 1 and 2 should be committed. Migration 3 should not be committed.
    ...
    func testManualEraseDatabaseOnSchemaChange() throws {
        ...
        // ... unless database gets erased
        if try dbQueue.read(migrator2.hasSchemaChanges) { try dbQueue.erase() }
        try migrator2.migrate(dbQueue)
        try XCTAssertFalse(dbQueue.read(migrator2.hasSchemaChanges))
        try XCTAssertEqual(dbQueue.read(migrator2.appliedMigrations), ["1", "2"])
    }
    ...
    func testMigrationForeignKeyChecks() throws {
        ...
        do {
            var migrator = DatabaseMigrator()
            migrator.registerMigration("A") { db in try db.execute(sql: foreignKeyViolation) }
            let dbQueue = try makeDatabaseQueue()
    ...
    func test_disablingDeferredForeignKeyChecks_applies_to_newly_registered_migrations_only() throws
    {
        ...
        migrator.registerMigration("B") { _ in XCTFail("Should not run") }
        let dbQueue = try makeDatabaseQueue()
    ...
    func test_schemaSource_is_disabled_during_migrations() throws {
        struct SchemaSource: DatabaseSchemaSource {
            func columnsForPrimaryKey(_: Database, inView _: DatabaseObjectID) throws -> [String]? {
                ["id"]
            }
        }
        ...
        do {
            migrator.registerMigration("A") { db in
                ...
                try XCTAssertThrowsError(db.primaryKey("myView"))
            }
            try migrator.migrate(dbQueue)
        }
        ...
        try dbQueue.inDatabase { db in
            ...
            try XCTAssertNoThrow(db.primaryKey("myView"))
        }
    ...
    func test_merged_migrations_named_like_the_last() throws {
        ...
        oldMigrator.registerMigration("v1") { db in try db.execute(sql: "CREATE TABLE t1(a)") }
        oldMigrator.registerMigration("v2") { db in try db.execute(sql: "CREATE TABLE t2(a)") }
        oldMigrator.registerMigration("v3") { db in try db.execute(sql: "CREATE TABLE t3(a)") }
        oldMigrator.registerMigration("v4") { db in try db.execute(sql: "CREATE TABLE t4(a)") }
        oldMigrator.registerMigration("v5") { db in try db.execute(sql: "CREATE TABLE t5(a)") }

        // New migrator merges v2, v3, and v4 into v4
        var newMigrator = DatabaseMigrator()
        newMigrator.registerMigration("v1") { db in try db.execute(sql: "CREATE TABLE t1(a)") }
        newMigrator.registerMigration("v4", merging: ["v2", "v3"]) { db, appliedIDs in
            if !appliedIDs.contains("v2") { try db.execute(sql: "CREATE TABLE t2(a)") }
            if !appliedIDs.contains("v3") { try db.execute(sql: "CREATE TABLE t3(a)") }
            try db.execute(sql: "CREATE TABLE t4(a)")
        }
        newMigrator.registerMigration("v5") { db in try db.execute(sql: "CREATE TABLE t5(a)") }
    ...
    func test_merged_migrations_with_a_new_name() throws {
        ...
        newMigrator.registerMigration("v4bis", merging: ["v2", "v3", "v4"]) { db, appliedIDs in
            if !appliedIDs.contains("v2") { try db.execute(sql: "CREATE TABLE t2(a)") }
            if !appliedIDs.contains("v3") { try db.execute(sql: "CREATE TABLE t3(a)") }
            if !appliedIDs.contains("v4") { try db.execute(sql: "CREATE TABLE t4(a)") }
        }
        newMigrator.registerMigration("v5") { db in try db.execute(sql: "CREATE TABLE t5(a)") }
