...
extension Database {
    ...
    struct SchemaCache {
        /// The available schema identifiers, in the order of SQLite resolution: temp, main, then
        /// attached databases.
    ...
    ///
    /// If the database schema is modified by another SQLite connection to the same database file,
    /// your application may need to call this method in order to avoid undesired consequences.
    public func clearSchemaCache() {
        ...
        // We also clear statement cache despite the automatic statement recompilation (see
        // https://www.sqlite.org/c3ref/prepare.html) because the automatic statement recompilation
        // only happens a limited number of times (`SQLITE_MAX_SCHEMA_RETRY`).
        internalStatementCache.clear()
        publicStatementCache.clear()
    }

    /// Clears the database schema cache if the database schema has changed since this method was
    /// last called.
    func clearSchemaCacheIfNeeded() throws {
        // `PRAGMA schema_version` fetches a 4-bytes integer (Int32), stored at offset 40 of the
        // database header: <https://sqlite.org/pragma.html#pragma_schema_version>
    ...
    /// Fetches the list of database schemas, in the order of SQLite resolution: temp, main, then
    /// attached databases (as documented at <https://www.sqlite.org/lang_naming.html>).
    func fetchSchemaIdentifiers() throws -> [DatabaseSchemaID] {
        if let schemaIDs = schemaCache.schemaIDs { return schemaIDs }

        var schemaIDs = try Array(
            Row
                .fetchCursor(self, sql: "PRAGMA database_list")
                .map { row -> DatabaseSchemaID in DatabaseSchemaID(name: row[1] as String) })

        // Temp schema shadows all other schemas: put it first
        if let tempIdx = schemaIDs.firstIndex(of: .temp) { schemaIDs.swapAt(tempIdx, 0) }
    ...
    /// Returns information about a table or a view
    func table(for table: DatabaseObjectID) throws -> TableInfo? { try _table(for: table) }
    #endif
    /// Returns information about a table or a view
    private func _table(for table: DatabaseObjectID) throws -> TableInfo? {
        ...
        if let tableInfo = schemaCache[table.schemaID].table(table.name) { return tableInfo.value }
    ...
    ///
    /// When `schemaName` is not specified, the result is true if any known schema contains the
    /// table.
    ///
    /// For example:
    ///
    ...
    public func tableExists(_ name: String, in schemaName: String? = nil) throws -> Bool {
        if let schemaName { return try exists(type: .table, name: name, in: schemaName) }
    ...
    public static func isSQLiteInternalTable(_ tableName: String) -> Bool {
        ...
        // > The names of internal schema objects always begin with "sqlite_" and any table, index,
        //   view, or trigger whose name begins with "sqlite_" is an internal schema object. SQLite
        //   prohibits applications from creating objects whose names begin with "sqlite_".
    ...
    public func viewExists(_ name: String, in schemaName: String? = nil) throws -> Bool {
        if let schemaName { return try exists(type: .view, name: name, in: schemaName) }

        return try fetchSchemaIdentifiers().contains { try exists(type: .view, name: name, in: $0) }
    }
    ...
    public func triggerExists(_ name: String, in schemaName: String? = nil) throws -> Bool {
        if let schemaName { return try exists(type: .trigger, name: name, in: schemaName) }
    ...
    ///
    /// This is checking for the existence of the entity specified by `type` and `name`. It is
    /// assumed that the existence of a schema named `schemaName` is already known and will throw an
    ...
    ///
    /// All tables have a primary key, even when it is not explicit. When a table has no explicit
    /// primary key, the result is the hidden "rowid" column.
    ///
    /// For example:
    ///
    /// ```swift
    ...
    /// ```
    ///
    /// When `schemaName` is nil, known schemas are iterated in SQLite resolution order, and the
    /// first matching result is returned. For more information, see
    /// <https://www.sqlite.org/lang_naming.html>.
    ///
    /// Database views are supported, if the connection is configured with a schema source. See
    /// ``Configuration/schemaSource``.
    ///
    /// - throws: A ``DatabaseError`` whenever an SQLite error occurs, if
    /// the specified schema does not exist, or if no such table exists in the main or temp schema,
    /// or in an attached database.
    public func primaryKey(
        ...
        if let schemaName {
            return try introspect(
                tableNamed: tableName, inSchemaNamed: schemaName, using: primaryKey(_:))
        }
    ...
    ///
    /// A fatal error is raised if the primary key has several columns, or if `tableName` is the
    /// name of a database view that is not customized with the schemaSource.
    func filteringPrimaryKeyColumn(_ tableName: String) throws -> String {
        do {
            ...
            // Maybe the user tries to filter a view by primary key, as in
            // <https://github.com/groue/GRDB.swift/issues/1648>. In this case, raise a fatalError
            // because this is a programmer error which is very likely to be detected during
            // development.
    ...
    private func primaryKey(_ table: DatabaseObjectID) throws -> PrimaryKeyInfo? {
        ...
        if primaryKey == nil { primaryKey = try fetchPrimitivePrimaryKey(forTable: table) }
    ...
    /// Fetches the primary key for the database table identified by `table`, or returns nil if the
    /// database schema does not contain that table.
    ///
    ...
    func fetchPrimitivePrimaryKey(forTable table: DatabaseObjectID) throws -> PrimaryKeyInfo? {
        ...
        // >
        // > This pragma returns one row for each column in the named table. Columns in the result
        //   set include the column name, data type, whether or not the column can be NULL, and the
        //   default value for the column. The "pk" column in the result set is zero for columns
        //   that are not part of the primary key, and is the index of the column in the primary key
        ...
        //
        // cid | name | type | notnull | dflt_value | pk | 0 | id | INTEGER | 0 | NULL | 1 | 1 |
        // name | TEXT | 0 | NULL | 0 | 2 | score | INTEGER | 0 | NULL | 0 |
        ...
        switch pkColumns.count {
            ...
            case 1:
                ...
                return pkColumn.type.uppercased() == "INTEGER"
                    ? .rowID(pkColumn)
                    : try .regular([pkColumn], tableHasRowID: fetchTableHasRowID(table))
    ...
    /// Fetches a customized primary key for the database view identified by `view`.
    ///
    /// Nil is returned in the view does not exist, or if `columns` is empty.
    ///
    /// - precondition: If the view exists, columns must exist, must identify a unique row, and must
    ///   be not null.
    func fetchPrimitivePrimaryKey(
        ...
        let pkInfos = columns.enumerated().map { index, column in
            guard var info = infos.first(where: { $0.name.lowercased() == column.lowercased() })
            else {
                // The requested column does not exist in the database schema. Is it a programmer
                // error, or a runtime error? Let's make it a programmer error to start with.
                fatalError(
                    """
            ...
            // Rewrite primaryKeyIndex, and use a 1-based index, as documented in
    ...
    private func fetchTableHasRowID(_ table: DatabaseObjectID) throws -> Bool {
        ...
        // To check if the table has a rowid, we compile a statement that selects the `rowid`
        // column. If compilation fails, we assume that the table is WITHOUT ROWID. This is not a
        // very robust test (users may create WITHOUT ROWID tables with a `rowid` column), but
        // nobody has reported any problem yet.
        //
        // Since compilation may fail, we may feed the SQLite error log, and users may wonder what
        // are those errors. That's why we use a distinctive alias
        // (`checkWithoutRowidOptimization`), so that anyone can search the GRDB code, find this
        // documentation, and understand why this query appears in the error log:
        // <https://github.com/groue/GRDB.swift/issues/945#issuecomment-804896196>
        //
        // We don't use `try makeStatement(sql:)` in order to avoid throwing an error (this annoys
        // users who set a breakpoint on Swift errors).
        let sql =
    ...
    ///
    /// SQLite does not define any index for INTEGER PRIMARY KEY columns: this method does not
    /// return any index that represents the primary key.
    ///
    /// If you want to know if a set of columns uniquely identifies a row, because the columns
    /// contain the primary key or a unique index, use ``table(_:hasUniqueKey:)``.
    ///
    /// When `schemaName` is nil, known schemas are iterated in SQLite resolution order, and the
    /// first matching result is returned. For more information, see
    /// <https://www.sqlite.org/lang_naming.html>.
    ///
    /// - throws: A ``DatabaseError`` whenever an SQLite error occurs, if
    /// the specified schema does not exist, or if no such table or view with this name exists in
    ...
    public func indexes(on tableName: String, in schemaName: String? = nil) throws -> [IndexInfo] {
        if let schemaName {
            return try introspect(
                tableNamed: tableName, inSchemaNamed: schemaName, using: indexes(on:))
        }
            ...
            .compactMap { row -> IndexInfo? in
                ...
                for indexInfoRow in indexInfoRows {
                    guard let column = indexInfoRow[2] as String? else {
                        ...
                        // > The name of the column being indexed is NULL if the column is the rowid
                        //   or an expression.
                        //
                        // IndexInfo does not support expressing such index. Maybe in a future GRDB
                        // version?
                        return nil
                    }
                ...
                return IndexInfo(
                    name: indexName, columns: columns, isUnique: unique, origin: origin)
            }

        if indexes.isEmpty {
            // PRAGMA index_list doesn't throw any error when table does not exist. So let's check
            // if table exists:
    ...
    ///
    /// The result is true if and only if the primary key, or a unique index, is included in the
    /// sequence.
    ///
    /// For example:
    ///
    ...
    ) throws -> Bool { try columnsForUniqueKey(columns, in: tableName) != nil }
    ...
    public func foreignKeys(
        ...
        for schemaID in try fetchSchemaIdentifiers() {
            if let result = try foreignKeys(on: DatabaseObjectID(
                name: tableName, schemaID: schemaID)) { return result }
        }
        throw DatabaseError.noSuchTable(tableName)
    }
    ...
    private func foreignKeys(on table: DatabaseObjectID) throws -> [ForeignKeyInfo]? {
        ...
        for row in try Row.fetchAll(
            ...
            sql: """
                ...
                """)
        {
            // row = [id:0 seq:0 table:"parents" from:"parentId" to:"id" on_update:"..."
            // on_delete:"..." match:"..."]
            let id: Int = row[0]
            let seq: Int = row[1]
        ...
        if rawForeignKeys.isEmpty {
            // PRAGMA foreign_key_list doesn't throw any error when table does not exist. So let's
        ...
        let foreignKeys = try rawForeignKeys.map {
            ...
            return ForeignKeyInfo(
                id: id, destinationTable: destinationTable, mapping: completeMapping)
        }
    ...
    ///
    /// When `schemaName` is not specified, known schemas are checked in SQLite resolution order and
    /// the first matching table is used.
    ///
    /// - throws: A ``DatabaseError`` whenever an SQLite error occurs, if
    /// the specified schema does not exist, or if no such table or view with this name exists in
    /// the main or temp schema, or in an attached database.
        ...
        throws -> RecordCursor<ForeignKeyViolation>
    {
        if let schemaName {
            ...
            if try exists(type: .table, name: tableName, in: schemaID) {
                return try foreignKeyViolations(in: DatabaseObjectID(
                    name: tableName, schemaID: schemaID))
            } else {
                throw DatabaseError.noSuchTable(tableName)
            }
        }
        ...
        for schemaID in try fetchSchemaIdentifiers()
            where try exists(type: .table, name: tableName, in: schemaID)
        {
            return try foreignKeyViolations(in: DatabaseObjectID(
                name: tableName, schemaID: schemaID))
        }
    ...
    private func foreignKeyViolations(
        in table: DatabaseObjectID
    ) throws -> RecordCursor<ForeignKeyViolation> {
    ...
    ///
    /// On the first foreign key violation found in the database, this method throws a
    /// ``DatabaseError`` with extended code `SQLITE_CONSTRAINT_FOREIGNKEY`.
    ///
    /// If you are looking for the list of foreign key violations, prefer ``foreignKeyViolations()``
    /// instead.
    public func checkForeignKeys() throws { try checkForeignKeys(from: foreignKeyViolations()) }
    ...
    /// the first matching table is used.
    ///
    /// On the first foreign key violation found in the table, this method throws a
    /// ``DatabaseError`` with extended code `SQLITE_CONSTRAINT_FOREIGNKEY`.
    ///
    ...
    /// specified schema does not exist; if no such table or view with this name exists in the main
    /// or temp schema or in an attached database.
    ...
    private func checkForeignKeys(from violations: RecordCursor<ForeignKeyViolation>) throws {
        if let violation = try violations.next() { throw violation.databaseError(self) }
    }

    /// Returns the actual name of the database table, or nil if the table does not exist.
    ///
    /// - throws: A ``DatabaseError`` whenever an SQLite error occurs, or if no
    /// such table exists in the main or temp schema, or in an attached database.
    func canonicalTableName(_ tableName: String) throws -> String? {
        for schemaID in try fetchSchemaIdentifiers() {
            ...
            if tableName.lowercased() == schemaTableName.lowercased() { return schemaTableName }
        }
        return nil
    }

    func schema(_ schemaID: DatabaseSchemaID) throws -> SchemaInfo {
        if let schemaInfo = schemaCache[schemaID].schemaInfo { return schemaInfo }
        let schemaInfo = try SchemaInfo(self, schemaTableName: schemaID.schemaTableName)
        schemaCache[schemaID].schemaInfo = schemaInfo
        return schemaInfo
    }

    /// Attempts to perform a table introspection function on a given table and schema.
    ///
    /// - parameter tableName: The name of the table to examine
    /// - parameter schemaName: The name of the schema to check
    /// - parameter introspector: An introspection function taking a `DatabaseObjectID` as the only
    ///   parameter. It the result is nil, introspection fails and this method throws
...
extension Database {
    ...
    ///
    /// - throws: A ``DatabaseError`` whenever an SQLite error occurs, if
    /// the specified schema does not exist,or if no such table or view with this name exists in the
    /// main or temp schema, or in an attached database.
    public func columns(in tableName: String, in schemaName: String? = nil) throws -> [ColumnInfo] {
        if let schemaName {
            return try introspect(
                tableNamed: tableName, inSchemaNamed: schemaName, using: columns(in:))
        }
    ...
    private func columns(in table: DatabaseObjectID) throws -> [ColumnInfo]? {
        ...
        if var columns = try fetchPrimitiveColumns(in: table) {
            // Discard hidden columns, so that the result of this method matches the columns in
            // `SELECT *`, and avoids surprises.
    ...
    /// Fetches the columns in the database table identified by `table`, or returns nil if the
    /// database schema does not contain that table.
    ///
    /// This method relies entirely on SQLite schema introspection. Starting SQLite 3.26.0, it
    /// returns all columns, including [generated columns](https://www.sqlite.org/gencol.html) and
    ...
    func fetchPrimitiveColumns(in table: DatabaseObjectID) throws -> [ColumnInfo]? {
        ...
        //   for columns that are part of the primary key.
        //
        // sqlite> CREATE TABLE players ( id INTEGER PRIMARY KEY, firstName TEXT, lastName TEXT);
        //
        // sqlite> PRAGMA table_info("players"); cid | name | type | notnull | dflt_value | pk | 0 |
        // id | INTEGER | 0 | NULL | 1 | 1 | name | TEXT | 0 | NULL | 0 | 2 | score | INTEGER | 0 |
        // NULL | 0 |
        //
        //
        // PRAGMA table_info does not expose hidden and generated columns. For that, we need PRAGMA
        // table_xinfo, introduced in SQLite 3.26.0: https://sqlite.org/releaselog/3_26_0.html
        //
        // > PRAGMA schema.table_xinfo(table-name);
        //
        // > This pragma returns one row for each column in the named table, including hidden
        //   columns in virtual tables. The output is the same as for PRAGMA table_info except that
        //   hidden columns are shown rather than being omitted.
        //
        // sqlite> PRAGMA table_xinfo("players"); cid | name | type | notnull | dflt_value | pk |
        // hidden 0 | id | INTEGER | 0 | NULL | 1 | 0 1 | firstName | TEXT | 0 | NULL | 0 | 0 2 |
        // lastName | TEXT | 0 | NULL | 0 | 0
        let columnInfoQuery: String
        columnInfoQuery = sqlite3_libversion_number() < 3_026_000
    ...
    /// If there exists a unique key that contains those columns, this method returns the columns of
    /// the unique key, ordered as the matching index (or primary key). The case of returned columns
    /// is not guaranteed to match the case of input columns.
    ///
    /// This method accepts both tables and views. For views, the primary key returned by the
    /// schemaSource is considered as a unique key.
    func columnsForUniqueKey(
        ...
        if let primaryKey = try? self.primaryKey(tableName) {
            if primaryKey.tableHasRowID, lowercasedColumns == ["rowid"] { return ["rowid"] }
        ...
        if try tableExists(tableName) {
            let matchingIndex = try indexes(on: tableName).first { index in
                index.isUnique
                    && Set(index.columns.map { $0.lowercased() }).isSubset(of: lowercasedColumns)
            }
            if let matchingIndex { return matchingIndex.columns }
        }
    ...
    func existenceCheckColumns(in tableName: String) throws -> [String] {
        do {
            ...
            if let rowIDColumn = primaryKey.rowIDColumn {
                ...
                //
                // // CREATE TABLE player (id INTEGER PRIMARY KEY, ...) try
                // db.existenceCheckColumns(in: "player") // ["id"]
                return [rowIDColumn]
            } else if primaryKey.tableHasRowID {
                // Prefer the rowid
                //
                // // CREATE TABLE player (uuid TEXT NOT NULL PRIMARY KEY, ...) try
                // db.existenceCheckColumns(in: "player") // ["rowid"]
                return [Column.rowID.name]
            } else {
                // WITHOUT ROWID table: use primary key columns
                //
                // // CREATE TABLE player (uuid TEXT NOT NULL PRIMARY KEY, ...) WITHOUT ROWID try
                // db.existenceCheckColumns(in: "player") // ["uuid"]
                return primaryKey.columns
            }
        } catch let error as DatabaseError {
            if case .SQLITE_ERROR = error.resultCode,
...
public struct DatabaseSchemaID: Hashable, Sendable {
    ...
    private init(impl: Impl) { self.impl = impl }
    ...
    public static func attached(_ name: String) -> DatabaseSchemaID { .init(impl: .attached(name)) }
    ...
    public var schemaTableName: String {
        switch impl {
            case .main: "sqlite_master"
            case .temp: "sqlite_temp_master"
            case let .attached(name): "\(name).sqlite_master"
        }
    }
    ...
    var unqualifiedSchemaTableName: String {
        switch impl {
            case .main, .attached: "sqlite_master"
            case .temp: "sqlite_temp_master"
        }
    }
    ...
    func viewNameInErrorMessages(_ viewName: String) -> String {
        switch impl {
            case .main: "view \(viewName)"
            case .temp: "temporary view \(viewName)"
            case let .attached(schemaName): "view \(schemaName).\(viewName)"
        }
    }
}
...
public struct DatabaseObjectID: Hashable, Sendable {
    ...
    /// Returns a quoted version of the identifier, for safe insertion in an SQL query.
    ///
    /// For example:
    ///
    /// ```
    ...
    /// ```
    var quotedDatabaseIdentifier: String { "\(schemaID.name).\(name.quotedDatabaseIdentifier)" }
}

/// Information about a column of a database table.
///
/// You get `ColumnInfo` instances with the ``Database/columns(in:in:)`` `Database` method.
///
...
public struct ColumnInfo: FetchableRecord, Sendable {
    ...
    ///
    /// The casing of this string depends on the SQLite version: make sure you process this string
    ...
    ///
    /// The casing of the raw value depends on the SQLite version: make sure you process the result
    /// in a case-insensitive way.
    var columnType: Database.ColumnType? {
        type.isEmpty ? nil : Database.ColumnType(rawValue: type)
    }
    ...
    ///
    /// When not nil, it contains an SQL string that defines an expression. That expression may be a
    /// literal, as `1`, or `'foo'`. It may also contain a non-constant expression such as
    /// `CURRENT_TIMESTAMP`.
    ///
    /// Related SQLite documentation:
...
///
/// You get `IndexInfo` instances with the ``Database/indexes(on:in:)`` `Database` method.
///
/// Related SQLite documentation:
///
...
/// ``Database/foreignKeyViolations()`` and ``Database/foreignKeyViolations(in:in:)`` methods.
///
/// For example:
///
/// ```swift
...
public struct ForeignKeyViolation: Sendable {
    ...
    ///
    /// This id matches the ``ForeignKeyInfo/id`` property in ``ForeignKeyInfo``. See
    ...
    public func failureDescription(_ db: Database) throws -> String {
        ...
        let originRow = try originRowID.flatMap { rowid in
            try Row.fetchOne(
                db,
                sql: "SELECT * FROM \(originTable.quotedDatabaseIdentifier) WHERE rowid = \(rowid)")
        }
        ...
        } else if let originRowID { description += ", in rowid \(originRowID)" }
    ...
    public func databaseError(_ db: Database) -> DatabaseError {
        // Grab detailed information, if possible, for better error message. If detailed information
        // is not available, fallback to plain description.
        let message = (try? failureDescription(db)) ?? String(describing: self)
        return .init(resultCode: .SQLITE_CONSTRAINT_FOREIGNKEY, message: message)
    }
}
...
///
/// You get `PrimaryKeyInfo` instances with the ``Database/primaryKey(_:in:)`` `Database` method.
///
...
public struct PrimaryKeyInfo: Sendable {
    private enum Impl {
        ...
        /// An INTEGER PRIMARY KEY column that aliases the Row ID. Associated string is the column
        /// name.
        case rowID(ColumnInfo)

        /// Any primary key, but INTEGER PRIMARY KEY. Associated strings are column names.
    ...
    public var columns: [String] {
        switch impl {
            case .hiddenRowID: [Column.rowID.name]
            case let .rowID(columnInfo): [columnInfo.name]
            case let .regular(columnInfos: columnInfos, tableHasRowID: _):
                columnInfos.map(\.name)
        }
    }

    /// The columns in the primary key. Nil if the primary key is the hidden rowID. Never empty
    /// otherwise.
    var columnInfos: [ColumnInfo]? {
        switch impl {
            case .hiddenRowID: nil
            case let .rowID(columnInfo): [columnInfo]
            case let .regular(columnInfos: columnInfos, tableHasRowID: _): columnInfos
        }
    }
    ...
    var tableHasRowID: Bool {
        switch impl {
            case .hiddenRowID: true
            case .rowID: true
            case let .regular(columnInfos: _, tableHasRowID: tableHasRowID): tableHasRowID
        }
    }
    ...
    var fastPrimaryKeyColumn: String? {
        if let rowIDColumn {
            ...
            //
            // // CREATE TABLE player (id INTEGER PRIMARY KEY, ...) try
            // db.primaryKey("player").fastPrimaryKeyColumn // "id"
            rowIDColumn
        } else if tableHasRowID {
            // Prefer the rowid
            //
            // // CREATE TABLE player (uuid TEXT NOT NULL PRIMARY KEY, ...) try
            // db.primaryKey("player").fastPrimaryKeyColumn // "rowid"
            Column.rowID.name
        } else if columns.count == 1 {
            // WITHOUT ROWID table or view customized with the schemaSource: use primary key column
            //
            // // CREATE TABLE player (uuid TEXT NOT NULL PRIMARY KEY, ...) WITHOUT ROWID try
            // db.primaryKey("player").fastPrimaryKeyColumn // "uuid"
            columns[0]
        } else {
            // WITHOUT ROWID table or view customized with the schemaSource with a multi-columns
            // primary key
            nil
        }
    }
...
///
/// You get `ForeignKeyInfo` instances with the ``Database/foreignKeys(on:in:)`` `Database` method.
///
/// Related SQLite documentation:
...
public struct ForeignKeyInfo: Sendable {
    ...
    /// The origin columns.
    public var originColumns: [String] { mapping.map(\.origin) }

    /// The destination columns.
    public var destinationColumns: [String] { mapping.map(\.destination) }
}
...
struct SchemaInfo: Equatable {
    ...
    /// Returns whether there exists a object of given type with this name (case-insensitive).
    func containsObjectNamed(_ name: String, ofType type: SchemaObjectType) -> Bool {
        let name = name.lowercased()
        return objects.contains { $0.type == type && $0.name.lowercased() == name }
    }

    /// Returns the canonical name of the object:
    ///
    /// try db.execute(sql: "CREATE TABLE FooBar (...)") try db.schema().canonicalName("foobar",
    /// ofType: .table) // "FooBar"
