...
extension MutablePersistableRecord {
    ...
    /// Executes an `INSERT` statement, and DOES NOT run insertion callbacks.
    @usableFromInline
        ...
        throws -> InsertionSuccess
    {
        ...
        if primaryKey.tableHasRowID {
            ...
            let rowid: Int64 = try withExtendedLifetime(cursor) { cursor in
                ...
                // Now that we have fetched the values we need, we could stop there. But let's make
                // sure we fully consume the cursor anyway, until SQLITE_DONE. This is necessary,
                // for example, for upserts in tables that are synchronized with an FTS5 table. See
                // <https://github.com/groue/GRDB.swift/issues/1390>
                while try cursor.next() != nil {}

                return rowid
            }

            // Update the persistenceContainer with the inserted rowid. This allows the Record class
            // to set its `hasDatabaseChanges` property to false in its `aroundInsert` callback.
            ...
            // Let's make sure we fully consume the cursor, until SQLITE_DONE. This is necessary,
            // for example, for upserts in tables that are synchronized with an FTS5 table. See
            // <https://github.com/groue/GRDB.swift/issues/1390>
            while try cursor.next() != nil {}
    ...
    /// Executes an `INSERT RETURNING` statement, and DOES NOT run insertion callbacks.
    @usableFromInline
