...
public extension Database {
    ...
    ///   - stream: A stream for text output, which directs output to the console by default.
    func dumpSQL(
        _ sql: SQL,
    ...
    ///   - stableOrder: A boolean value that controls the ordering of rows fetched from views. If
    ///     false (the default), rows are printed in the order specified by the view (which may be
    ///     undefined). It true, outputted rows are always printed in the same stable order. The
    ///     purpose of this stable order is to make the output suitable for testing.
    ///   - stream: A stream for text output, which directs output to the console by default.
    func dumpTables(
        _ tables: [String],
        format: some DumpFormat = .debug(),
    ...
    /// ```
    ///
    /// This prints the database schema as well as the content of all tables. For example:
    ///
    /// ```
    /// sqlite_master
    ...
    /// ```
    ///
    /// > Note: Internal SQLite and GRDB schema objects are not recorded (those with a name that
    ///   starts with "sqlite_" or "grdb_").
    /// >
    /// > [Shadow tables](https://www.sqlite.org/vtab.html#xshadowname) are not recorded, starting
    ///   SQLite 3.37+.
    ///
    /// - Parameters:
    ///   - format: The output format.
...
extension Database {
        ...
        throws
    {
        ...
        if let supplementaryFetch = preparedRequest.supplementaryFetch {
            ...
            // sm:ignore:next useTrailingClosures the multi-trailing form fails to type check
        ...
        throws
    {
        ...
        for table in tables {
            ...
            if header { stream.writeln(table) }
        ...
        throws
    {
        ...
        let tables = try String
            ...
            .filter { try !ignoresObject(named: $0) }
        try _dumpTables(
            tables, format: format, tableHeader: .always, stableOrder: true, to: &stream)
    }
        ...
        throws
    {
        ...
        for row in sqlRows {
            ...
            if try ignoresObject(named: name) { continue }
            stream.writeln(row[0])
        }
    }

    private func ignoresObject(named name: String) throws -> Bool {
        Database.isSQLiteInternalTable(name)
            ? true
            : Database.isGRDBInternalTable(name) ? true : try isShadowTable(name) ? true : false
    }
...
public enum DumpTableHeaderOptions: Sendable { case automatic, always }
