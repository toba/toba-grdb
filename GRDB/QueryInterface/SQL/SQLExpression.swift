/// An SQL expression.
///
/// `SQLExpression` is an opaque representation of an SQL expression. You generally build
/// `SQLExpression` from other expressions. For example:
///
/// ```swift
/// // Values
/// 1000.sqlExpression
...
/// ```
///
/// `SQLExpression` is better used as the return type of a function. For function arguments, prefer
/// the ``SQLExpressible`` or ``SQLSpecificExpressible`` protocols. For example:
///
/// ```swift
...
public struct SQLExpression: Sendable {
    ...
    /// The preferred interpretation of the expression in JSON building contexts (see
    /// `jsonBuilderExpression`).
    ///
    /// ```swift
    /// // Considering:
    ...
    private enum Impl {
        ...
        ///
        /// <lhs> LIKE <rhs> <lhs> LIKE <rhs> ESCAPE <escape>
        ...
        ///
        /// <expr1> AND <expr2> <expr1> + <expr2> + <expr3>
        ///
        ...
        ///
        /// <expression> IN <collection> <expression> NOT IN <collection>
        ...
        ///
        /// <lhs> = <rhs> <lhs> <> <rhs> <lhs> IS <rhs> <lhs> IS NOT <rhs>
        ...
        /// A function call.
        ///
        /// <function>(<argument>, ...) <function>(DISTINCT <argument>)
        ...
        ///
        /// It crashes for WITHOUT ROWID table with a multi-columns primary key. Future versions of
        /// GRDB may use [row values](https://www.sqlite.org/rowvalue.html).
        ///
        /// id rowid code
        case fastPrimaryKey
        ...
        ///
        /// player.id document.rowid country.code
        ...
        func qualified(with alias: TableAliasBase) -> Impl {
            switch self {
                ...
                case let .column(name): .qualifiedColumn(name, alias)

                case let .rowValue(expressions):
                    .rowValue(expressions.map { $0.qualified(with: alias) })

                case let .literal(sqlLiteral): .literal(sqlLiteral.qualified(with: alias))

                case let .cast(expression, storageClass):
                    .cast(expression.qualified(with: alias), storageClass)
                ...
                case let .binary(op, lhs, rhs):
                    .binary(op, lhs.qualified(with: alias), rhs.qualified(with: alias))
                ...
                case let .associativeBinary(op, expressions):
                    .associativeBinary(op, expressions.map { $0.qualified(with: alias) })
                ...
                case let .unary(op, expression): .unary(op, expression.qualified(with: alias))

                case let .compare(op, lhs, rhs):
                    .compare(op, lhs.qualified(with: alias), rhs.qualified(with: alias))

                case let .tableMatch(a, expression):
                    .tableMatch(a, expression.qualified(with: alias))

                case let .not(expression): .not(expression.qualified(with: alias))

                case let .collated(expression, collationName):
                    .collated(expression.qualified(with: alias), collationName)

                case .countAll: .countAll

                case let .simpleFunction(invocation):
                    .simpleFunction(invocation.qualified(with: alias))

                case let .aggregateFunction(invocation):
                    .aggregateFunction(invocation.qualified(with: alias))

                case let .isEmpty(expression, isNegated: isNegated):
                    .isEmpty(expression.qualified(with: alias), isNegated: isNegated)

                case .fastPrimaryKey: .qualifiedFastPrimaryKey(alias)
            }
        }
    }
    ...
    enum BooleanTest { case `true`, `false`, falsey }
    ...
    public struct AssociativeBinaryOperator: Hashable, Sendable {
        ...
        ///
        /// `+` and `*` are not stricly associative when applied to floating point values.
        let isStrictlyAssociative: Bool

        /// If true, (a • b) is a bijective function of a, and a bijective function of b.
        ///
        /// `||` (concat) is bijective.
        ///
        /// `AND`, `OR`, `+` and `*`, `&`, `|` are not.
        ...
        init(sql: String, neutralValue: DatabaseValue, strictlyAssociative: Bool, bijective: Bool) {
            ...
            isStrictlyAssociative = strictlyAssociative
    ...
    ///
    /// See also `AssociativeBinaryOperator`, `EqualityOperator`, `EscapableBinaryOperator`.
    struct BinaryOperator: Hashable {
        /// The SQL operator
        let sql: String

        /// A boolean value indicating if the operator is known to return a JSON value.
        ///
    ...
    /// `EscapableBinaryOperator` is an SQLite binary operator that accepts an `ESCAPE` clause, such
    /// as `LIKE`, etc.
    ///
    /// See also `AssociativeBinaryOperator`, `EqualityOperator`, `BinaryOperator`.
    struct EscapableBinaryOperator {
        ...
        ///
        /// let operator = BinaryOperator("LIKE", negated: "NOT LIKE") operator.negated!.sql // NOT
        /// LIKE
        var negated: EscapableBinaryOperator { .init(negatedSQL, negated: sql) }
    ...
    struct UnaryOperator: Hashable {
        ...
        init(_ sql: String) { self.sql = sql }
    ...
    /// Describes the interpretation of an expression in a JSON building context.
    enum JSONInterpretation { case deferredToSQLite, jsonValue }
}
...
extension SQLExpression {
    ...
    ///
    /// id name
    static func column(_ name: String) -> Self { self.init(impl: .column(name)) }
    ...
    static func rowValue(_ expressions: [SQLExpression]) -> Self? {
        guard let expression = expressions.first else { return nil }
        return expressions.count == 1
            ? expression
            : self.init(impl: .rowValue(expressions))
    }
    ...
    static func between(
        ...
        if case let .collated(expression, collationName) = expression.impl {
            // Prefer: expression BETWEEN lowerBound AND upperBound COLLATE collation over:
            // (expression COLLATE collation) BETWEEN lowerBound AND upperBound
            //
            // This transformation was introduced in GRDB v0.42.0, for the first release of the
            // query interface:
            ...
            // The commit is a big squash, and we've lost the original intent. It is likely just an
            // SQL aesthetic preference of mine.
            //
            // According to
            // https://www.sqlite.org/datatype3.html#assigning_collating_sequences_from_sql this
            // rewriting should not have any functional impact. Yet if any user complains
            // eventually, we should just remove this rewriting rule without any resistance.
            collated(
                between(
                    expression: expression,
                    lowerBound: lowerBound,
            ...
            self.init(impl: .between(
                expression: expression, lowerBound: lowerBound, upperBound: upperBound,
                isNegated: isNegated))
        }
    }
    ...
    /// A binary operator.
    ///
    /// <lhs> * <rhs> <lhs> <= <rhs> <lhs> LIKE <rhs>
    static func binary(_ op: BinaryOperator, _ lhs: SQLExpression, _ rhs: SQLExpression) -> Self {
        // See `between(expression:lowerBound:upperBound:isNegated:)` for some explanation of these
        // rewriting rules.
        if case let .collated(lhs, collationName) = lhs.impl {
            // Prefer: lhs <= rhs COLLATE collation over: (lhs COLLATE collation) <= rhs
            collated(binary(op, lhs, rhs), collationName)
        } else if case let .collated(rhs, collationName) = rhs.impl {
            // Prefer: lhs <= rhs COLLATE collation over: lhs <= (rhs COLLATE collation)
            collated(binary(op, lhs, rhs), collationName)
        } else {
    ...
    static func escapableBinary(
        ...
        _ rhs: SQLExpression,
        escape: SQLExpression?
    ) -> Self { self.init(impl: .escapableBinary(op, lhs, rhs, escape: escape)) }

    /// An associative binary operator.
    ///
    /// <expr1> AND <expr2> <expr1> + <expr2> + <expr3>
    ///
    /// When the `expressions` array is empty, returns the neutral value of the operator.
    ///
    /// When the `expressions` array contains a single expression, returns this expression.
    ///
    /// When the operator is strictly associative, the expressions are flattened:
    ///
    /// (a AND b) AND c -> a AND b AND c
    static func associativeBinary(
        _ op: AssociativeBinaryOperator,
        _ expressions: [SQLExpression]
    ) -> Self {
        ...
        guard let expression = expressions.first else { return op.neutralValue.sqlExpression }
        return expressions.count == 1
            ? expression : self.init(impl: .associativeBinary(op, expressions))
    }
    ...
    static func `in`(
        ...
        _ collection: SQLCollection,
        isNegated: Bool = false
    ) -> Self { self.init(impl: .in(expression, collection, isNegated: isNegated)) }
    ...
    static func compare(
        ...
        if case let .collated(lhs, collationName) = lhs.impl {
            // Prefer: lhs = rhs COLLATE collation over: (lhs COLLATE collation) = rhs
            collated(compare(op, lhs, rhs), collationName)
        } else if case let .collated(rhs, collationName) = rhs.impl {
            // Prefer: lhs = rhs COLLATE collation over: lhs = (rhs COLLATE collation)
            collated(compare(op, lhs, rhs), collationName)
        } else {
    ...
    ///
    /// <lhs> = <rhs> <lhs> IS NULL <rhs> IS NULL
    static func equal(_ lhs: SQLExpression, _ rhs: SQLExpression) -> Self {
        switch (lhs.impl, rhs.impl) {
            case let (impl, .databaseValue(.null)),
                ...
                // ... IS NULL
                compare(.is, SQLExpression(impl: impl), .null)
            default:
                // lhs = rhs
    ...
    ///
    /// NOT <expression>
    static func not(_ expression: SQLExpression) -> Self { self.init(impl: .not(expression)) }

    /// A collated expression.
    ///
    /// <expression> COLLATE <collation>
    static func collated(
        _ expression: SQLExpression,
        _ collationName: Database.CollationName
    ) -> Self {
        switch expression.impl {
            case let .in(expression, collection, isNegated: isNegated):
                ...
                .in(.collated(expression, collationName), collection, isNegated: isNegated)
            ...
            case let .associativeBinary(op, expressions):
                ...
                //      ("foo"..<"bar").contains(Column("name")).collating(.nocase)
                //      ("foo"..<"bar").contains(Column("name").collating(.nocase))
                .associativeBinary(op, expressions.map { $0.collating(collationName) })

            default: self.init(impl: .collated(expression, collationName))
        }
    }
    ...
    static func function(_ functionName: String, _ arguments: [SQLExpression]) -> Self {
        ...
        if (name == "MAX" || name == "MIN"), arguments.count > 1 {
            return .simpleFunction(functionName, arguments, isPure: true, isJSONValue: false)
    ...
    static func simpleFunction(
        ...
        .init(impl: .simpleFunction(SQLSimpleFunctionInvocation(
            name: name, arguments: arguments, isPure: isPure, isJSONValue: isJSONValue)))
    }
    ...
    static func aggregateFunction(
        ...
        .init(impl: .aggregateFunction(.init(
            name: name, arguments: arguments, isDistinct: isDistinct, ordering: ordering,
            filter: filter, isJSONValue: isJSONValue)))
    }

    /// The `COUNT` function.
    ///
    /// COUNT(<expression>)
    static func count(_ expression: SQLExpression) -> Self { function("COUNT", [expression]) }
...
extension SQLExpression {
    ...
    func qualified(with alias: TableAliasBase) -> Self {
        .init(
            impl: impl.qualified(with: alias),
            preferredJSONInterpretation: preferredJSONInterpretation)
    }
...
extension SQLExpression {
    ...
    ///
    /// This method makes it possible to avoid inserting `LIMIT 1` to the SQL of some requests:
    ///
    /// // SELECT * FROM "player" WHERE "id" = 1 try Player.fetchOne(db, key: 1) try
    /// Player.filter(Column("id") == 1).fetchOne(db)
    ///
    /// // SELECT * FROM "player" WHERE "name" = 'Arthur' LIMIT 1 try Player.filter(Column("name")
    /// == "Arthur").fetchOne(db)
    ///
    /// This method makes it possible to track individual rows identified by their row ids, and
    /// ignore modifications to other rows:
    ///
    /// // Track rows 1, 2, 3 only let request = Player.filter(keys: [1, 2, 3]) let
    /// regionObservation = DatabaseRegionObservation(tracking: request) let valueObservation =
    /// ValueObservation.tracking(request.fetchAll)
    ///
    /// - parameter acceptsBijection: If true, expressions that define a bijection on a column
    ///   return this column. For example: `-score` returns `score`.
    func column(
        _ db: Database,
        for alias: TableAliasBase,
        acceptsBijection: Bool = false
    ) throws -> String? {
        switch impl {
            case let .qualifiedColumn(name, a): return alias == a ? name : nil

            case let .associativeBinary(op, expressions):
                assert(expressions.count > 1)
                guard acceptsBijection, op.isBijective else { return nil }
                let nonConstants = expressions.filter { $0.isConstantInRequest == false }
                return nonConstants.count == 1
                    ? try nonConstants[0].column(db, for: alias, acceptsBijection: acceptsBijection)
                    : nil
            ...
            case let .qualifiedFastPrimaryKey(a):
                return alias == a ? try db.primaryKey(alias.tableName).fastPrimaryKeyColumn : nil
    ...
    ///
    /// - parameter context: An SQL generation context which accepts statement arguments.
    /// - parameter wrappedInParenthesis: If true, the returned SQL should be wrapped inside
    ///   parenthesis.
    func sql(_ context: SQLGenerationContext, wrappedInParenthesis: Bool = false) throws -> String {
        switch impl {
            case let .column(name): return name.quotedDatabaseIdentifier
            ...
            case let .databaseValue(dbValue):
                if dbValue.isNull {
                    ...
                    return try String.fetchOne(
                        context.db, sql: "SELECT QUOTE(?)", arguments: [dbValue])!
                }
            ...
            case let .literal(sqlLiteral):
                ...
                if wrappedInParenthesis { resultSQL = "(\(resultSQL))" }
                return resultSQL
            ...
            case let .simpleFunction(invocation): return try invocation.sql(context)
            ...
            case .fastPrimaryKey:
                // Likely a GRDB bug: how comes this expression is used before it has been qualified?
            ...
            case let .qualifiedFastPrimaryKey(alias):
                ...
                guard let column = primaryKey.fastPrimaryKeyColumn else {
                    fatalError(
                        "Not implemented for WITHOUT ROWID table with a multi-columns primary key")
                }
            ...
            case let .qualifiedExists(alias, isNegated: isNegated):
                ...
                let existenceCheckColumns = try context.db.existenceCheckColumns(
                    in: alias.tableName)
                return isNegated
    ...
    ///
    /// WHERE 0 -- [] WHERE a -- [] WHERE a = b -- [] WHERE a = 1 -- ["a"] WHERE a = 1 AND b = 2 --
    /// ["a", "b"] WHERE a = 1 AND b = 2 AND c > 0 -- ["a", "b"] WHERE a = 1 OR a = 2 -- [] WHERE a
    /// > 1 -- []
    ///
    /// This method makes it possible to avoid inserting `LIMIT 1` to the SQL of some requests:
    ///
                ...
                switch op {
                    case .equal, .is:
                        if let column = try lhs.column(db, for: alias, acceptsBijection: true),
                           rhs.isConstantInRequest { return [column] }

                        if let column = try rhs.column(db, for: alias, acceptsBijection: true),
                           lhs.isConstantInRequest { return [column] }

                        return []

                    case .notEqual, .isNot: return []
                }
    ...
    /// Returns the rowIds that identify rows in the request. A nil result means an unbounded list.
    ///
    /// When in doubt, returns nil.
    ///
    /// WHERE 1 -- nil WHERE 0 -- [] WHERE NULL -- [] WHERE id IS NULL -- [] WHERE id = 1 -- [1]
    /// WHERE id = 1 AND b = 2 -- [1] WHERE id = 1 OR id = 2 -- [1, 2] WHERE id IN (1, 2, 3) -- [1,
    /// 2, 3] WHERE id IN (1, 2) OR rowid IN (2, 3) -- [1, 2, 3] WHERE id > 1 -- nil
    ///
    /// This method makes it possible to track individual rows identified by their row ids, and
    /// ignore modifications to other rows:
    ///
    ...
    func identifyingRowIDs(_ db: Database, for alias: TableAliasBase) throws -> Set<Int64>? {
        switch impl {
            case let .databaseValue(dbValue):
                return dbValue.isNull || dbValue == false.databaseValue ? [] : nil
                   ...
                   try db.columnIsRowID(column, of: alias.tableName)
                {
                    return Set(expressions.compactMap { expression in
                        if case let .databaseValue(dbValue) = expression.impl {
                            Int64.fromDatabaseValue(dbValue)
                        } else {
                            nil
                        }
                    })
                ...
                switch op {
                    case .equal, .is:
                        if let column = try lhs.column(db, for: alias),
                           try db.columnIsRowID(column, of: alias.tableName),
                           case let .databaseValue(dbValue) = rhs.impl
                        {
                            if let rowID = Int64.fromDatabaseValue(dbValue) {
                                return [rowID]
                            } else {
                                // We miss `rowid = '1'` here, because SQLite would interpret the '1' string
                                // as a number
                                return []
                            }
                        }
                        ...
                        if let column = try rhs.column(db, for: alias),
                           try db.columnIsRowID(column, of: alias.tableName),
                           case let .databaseValue(dbValue) = lhs.impl
                        {
                            if let rowID = Int64.fromDatabaseValue(dbValue) {
                                return [rowID]
                            } else {
                    ...
                    case .notEqual, .isNot: return nil
                }
    ...
    ///
    /// The `= 1` and `= 0` tests allow the SQLite query planner to optimize queries with indices on
    /// boolean columns and expressions. See <https://github.com/groue/GRDB.swift/issues/816>
    ///
        ...
        switch impl {
            case let .databaseValue(dbValue):
                switch dbValue.storage {
                    ...
                    case let .int64(int64) where int64 == 0 || int64 == 1:
                        switch test {
                            case .true: (int64 == 1).sqlExpression
                            case .false, .falsey: (int64 == 0).sqlExpression
                        }

                    default:
                        switch test {
                            case .true: .compare(.equal, self, true.sqlExpression)
                            case .false: .compare(.equal, self, false.sqlExpression)
                            case .falsey: .not(self)
                        }
                }
            ...
            case let .escapableBinary(op, lhs, rhs, escape):
                switch test {
                    ...
                    case .falsey: .escapableBinary(op.negated, lhs, rhs, escape: escape)
                }
            ...
            case let .exists(subquery, isNegated: isNegated):
                switch test {
                    ...
                    case .falsey: SQLExpression(impl: .exists(subquery, isNegated: !isNegated))
                }
            ...
            case let .in(expression, collection, isNegated: isNegated):
                switch test {
                    ...
                    case .falsey: .in(expression, collection, isNegated: !isNegated)
                }
            ...
            case let .compare(op, lhs, rhs):
                switch test {
                    ...
                    case .falsey: .compare(op.negated, lhs, rhs)
                }
            ...
            case let .collated(expression, collationName):
                .collated(expression.is(test), collationName)

            case let .isEmpty(expression, isNegated: isNegated):
                switch test {
                    case .true: self
                    case .false, .falsey: .isEmpty(expression, isNegated: !isNegated)
                }
            ...
            case let .qualifiedExists(alias, isNegated: isNegated):
                switch test {
                    ...
                    case .falsey:
                        SQLExpression(impl: .qualifiedExists(alias, isNegated: !isNegated))
                }
    ...
    /// Returns true if the expression has a unique value when SQLite runs a request.
    ///
    /// When in doubt, returns false.
    ///
    /// 1 -- true 1 + 2 -- true score -- false
    ///
    /// This property supports `identifyingColums(_:for:)`
    var isConstantInRequest: Bool {
        switch impl {
            case .databaseValue: return true

            case let .rowValue(expressions),
                 let .associativeBinary(_, expressions):
                return expressions.allSatisfy(\.isConstantInRequest)

            case let .cast(expression, _): return expression.isConstantInRequest

            case let .between(
                expression: expression, lowerBound: lowerBound, upperBound: upperBound, isNegated: _
            ):
            ...
            case let .in(expression, collection, isNegated: _):
                guard let expressions = collection.collectionExpressions else { return false }
                return expression.isConstantInRequest
                    && expressions.allSatisfy(\.isConstantInRequest)

            case let .unary(_, expression),
                 let .not(expression),
                 let .isEmpty(expression, isNegated: _),
                 let .collated(expression, _): return expression.isConstantInRequest

            case let .simpleFunction(invocation): return invocation.isConstantInRequest
    ...
    /// When in doubt, returns false.
    ///
    /// SELECT score -- false SELECT COUNT(*) -- true SELECT MAX(score) -- true SELECT MAX(score) +
    /// 1 -- true
    ///
    /// This method makes it possible to avoid inserting `LIMIT 1` to the SQL of some requests:
    ///
    /// // SELECT MAX("score") FROM "player" try Player.select(max(Column("score")), as:
    /// Int.self).fetchOne(db)
    ///
    /// // SELECT "score" FROM "player" LIMIT 1 try Player.select(Column("score"), as:
    /// Int.self).fetchOne(db)
    var isAggregate: Bool {
        switch impl {
            ...
            case let .between(expression: expression, lowerBound: _, upperBound: _, isNegated: _),
                 ...
                 let .isEmpty(expression, isNegated: _): return expression.isAggregate

            case let .binary(_, lhs, rhs),
                 let .compare(_, lhs, rhs): return lhs.isAggregate || rhs.isAggregate
            ...
            case .countAll: return true

            case .aggregateFunction: return true
...
struct SQLSimpleFunctionInvocation {
    ...
    /// A boolean value indicating if a function is known to return a JSON value.
    ///
    /// A false value does not provide any information.
    var isJSONValue: Bool

    var isConstantInRequest: Bool { isPure && arguments.allSatisfy(\.isConstantInRequest) }
...
struct SQLAggregateFunctionInvocation {
    ...
    var filter: SQLExpression?  // SQLite 3.30+
    ...
    func sql(_ context: SQLGenerationContext, wrappedInParenthesis: Bool) throws -> String {
        ...
        if isDistinct { sql += "(DISTINCT " } else { sql += "(" }

        sql += try arguments
            .map { try $0.sql(context) }
            .joined(separator: ", ")

        if let ordering { sql += try " ORDER BY \(ordering.sql(context))" }

        sql += ")"

        if let filter { sql += try " FILTER (WHERE \(filter.sql(context)))" }

        if wrappedInParenthesis, filter != nil { return "(\(sql))" } else { return sql }
    }
}
...
extension SQLExpression {
    /// A boolean value indicating if the expression is known to be a JSON value.
    ///
    /// A false value does not provide any information.
    ///
    /// For examples:
    ...
    var isJSONValue: Bool {
        switch impl {
            case .databaseValue(.null): true

            case let .binary(op, _, _): op.isJSONValue

            case let .collated(expression, _): expression.isJSONValue

            case let .simpleFunction(invocation): invocation.isJSONValue

            case let .aggregateFunction(invocation): invocation.isJSONValue
...
public extension SQLExpressible where Self == Column {
    ...
    /// ```
    static var rowID: Self { Column.rowID }
}

/// A database-specific type that can be used as an SQL expression.
///
/// `SQLSpecificExpressible` is the protocol for all database-specific types that can be turned into
/// an SQL expression. Types whose existence is not purely dedicated to the database should adopt
/// the ``SQLExpressible`` protocol instead.
///
/// For example, ``Column`` is a type that only exists to help you build requests, and it adopts
...
public protocol SQLSpecificExpressible: SQLExpressible, SQLSelectable, SQLOrderingTerm {
    // SQLExpressible can be adopted by Swift standard types, and user types, through the
    // DatabaseValueConvertible protocol which inherits from SQLExpressible.
    //
    // For example, Int adopts SQLExpressible through DatabaseValueConvertible.
    //
    // SQLSpecificExpressible, on the other side, is not adopted by any Swift standard type or any
    // user type. It is only adopted by GRDB types, such as Column and SQLExpression.
    //
    // This separation lets us define functions and operators that do not spill out. The three
    // declarations below have no chance overloading a Swift-defined operator, or a user-defined
    // operator:
    //
...
public extension SQLSpecificExpressible {
    var sqlSelection: SQLSelection { .expression(sqlExpression) }

    var sqlOrdering: SQLOrdering { .expression(sqlExpression) }
}
...
public extension Sequence where Element: SQLSpecificExpressible {
    ...
    /// ```
    ///
    /// When the sequence is empty, `joined(operator:)` returns the neutral value of the operator.
    /// It is:
    ///
...
public extension Sequence<any SQLSpecificExpressible> {
    /// Returns an expression by joining all elements with an associative SQL binary operator.
    ///
    /// For example:
    ///
...
extension SQLSpecificExpressible {
    ...
    public var asc: SQLOrdering { .asc(sqlExpression) }

    /// An ordering term for descending order (nulls last).
    public var desc: SQLOrdering { .desc(sqlExpression) }

    #if GRDBCUSTOMSQLITE || SQLITE_HAS_CODEC
    /// An ordering term for ascending order (nulls last).
    public var ascNullsLast: SQLOrdering { .ascNullsLast(sqlExpression) }

    /// An ordering term for descending order (nulls first).
    public var descNullsFirst: SQLOrdering { .descNullsFirst(sqlExpression) }
    #else
    /// An ordering term for ascending order (nulls last).
    public var ascNullsLast: SQLOrdering { .ascNullsLast(sqlExpression) }
...
public extension SQLSpecificExpressible {
    ...
    /// ``ColumnExpression/detached``. For example:
    ///
    /// ```swift
    ...
    /// ```
    func forKey(_ key: String) -> SQLSelection { .aliasedExpression(sqlExpression, key) }

    /// Returns an aliased column with the same name as the coding key.
    ///
    /// See <doc:SQLSpecificExpressible/forKey(_:)-3xk0>.
    func forKey(_ key: some CodingKey) -> SQLSelection { forKey(key.stringValue) }
}
