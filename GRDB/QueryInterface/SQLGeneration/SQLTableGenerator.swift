struct SQLTableGenerator {
    ...
    /// Used for auto-referencing foreign keys: we need to know the columns of the primary key
    /// before they exist in the database schema, hence the name of "forward" primary key columns.
    ///
    ...
    func sql(_ db: Database) throws -> String {
        ...
        do {
            ...
            if options.contains(.temporary) { chunks.append("TEMPORARY") }
            chunks.append("TABLE")
            if options.contains(.ifNotExists) { chunks.append("IF NOT EXISTS") }
            ...
            do {
                ...
                for constraint in foreignKeyConstraints {
                    ...
                    if constraint.isDeferred { chunks.append("DEFERRABLE INITIALLY DEFERRED") }
                    items.append(chunks.joined(separator: " "))
                }
            ...
            if options.contains(.strict) { tableOptions.append("STRICT") }
            #else
            if options.contains(.strict) { tableOptions.append("STRICT") }
            #endif
            if options.contains(.withoutRowID) { tableOptions.append("WITHOUT ROWID") }

            if !tableOptions.isEmpty { chunks.append(tableOptions.joined(separator: ", ")) }
    ...
    init(_ db: Database, table: TableDefinition) throws {
            ...
            throws -> SQLTableGenerator.KeyConstraint
        {
            try SQLTableGenerator.KeyConstraint(
                columns: constraint.components.flatMap { component -> [String] in
                    switch component {
                        case let .columnName(columnName): [columnName]
                        case let .columnDefinition(column): [column.name]
                        case let .foreignKeyDefinition(foreignKey):
                            try Self.makeForeignKeyGenerator(
        ...
        for component in table.columnComponents {
            switch component {
                ...
                case let .columnLiteral(sql): columnGenerators.append(.columnLiteral(sql))
                ...
                case let .foreignKeyConstraint(constraint): foreignKeyConstraints.append(constraint)
            }
        }
        ...
        throws -> ForeignKeyGenerator
    {
        ...
        guard let primaryKeyColumns = destinationPrimaryKey.primaryKeyColumns else {
            ...
            switch foreignKey.indexing {
                case nil: break
                case .index: column.indexed()
                case .unique: column.unique()
            }
...
extension SQLColumnDescriptor {
    ...
    init(_ column: ColumnDefinition) { self.init(name: column.name, type: column.type) }
}
