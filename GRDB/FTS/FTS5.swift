...
public struct FTS5 {
    ...
    public enum Diacritics: Sendable {
        /// Do not remove diacritics from Latin script characters. This option matches the raw
        /// "remove_diacritics=0" tokenizer argument.
        case keep
        /// Remove diacritics from Latin script characters. This option matches the raw
        /// "remove_diacritics=1" tokenizer argument.
        case removeLegacy
        #if GRDBCUSTOMSQLITE || SQLITE_HAS_CODEC
        /// Remove diacritics from Latin script characters. This option matches the raw
        /// "remove_diacritics=2" tokenizer argument, available from SQLite 3.27.0
        case remove
        #else
        /// Remove diacritics from Latin script characters. This option matches the raw
        /// "remove_diacritics=2" tokenizer argument, available from SQLite 3.27.0
    ...
    // Support for FTS5Pattern initializers. Don't make public. Users tokenize with
    // `FTS5Tokenizer.tokenize()` methods, which support custom tokenizers, token flags, and
    // query/document tokenzation.
    /// Tokenizes the string argument as an FTS5 query.
    ///
    /// For example:
    ///
    /// try FTS5.tokenize(query: "SQLite database") // ["sqlite", "database"] try
    /// FTS5.tokenize(query: "Gustave Doré") // ["gustave", "doré"])
    ///
    /// Synonym (colocated) tokens are not present in the returned array. See `FTS5_TOKEN_COLOCATED`
    /// at <https://www.sqlite.org/fts5.html#custom_tokenizers> for more information.
    ///
    ...
    public static func api(_ db: Database) -> UnsafePointer<fts5_api> {
        ...
        guard code == SQLITE_OK else { fatalError("FTS5 is not available") }
        defer { sqlite3_finalize(statement) }
        type.utf8Start.withMemoryRebound(to: CChar.self, capacity: type.utf8CodeUnitCount) {
            typePointer in _ = sqlite3_bind_pointer(statement, 1, &api, typePointer, nil)
        }
        sqlite3_step(statement)
        guard let api else { fatalError("FTS5 is not available") }
        return api
    }
}
...
extension FTS5: VirtualTableModule {
    ...
    ) -> FTS5TableDefinition { .init(configuration: configuration) }
    ...
    public func moduleArguments(
        ...
        if let tokenizer = definition.tokenizer {
            let tokenizerSQL =
                try tokenizer
                .components
                .map { component in try component.sqlExpression.quotedSQL(db) }
                .joined(separator: " ")
                .sqlExpression
                .quotedSQL(db)
        ...
        if let columnSize = definition.columnSize { arguments.append("columnSize=\(columnSize)") }

        if let detail = definition.detail { arguments.append("detail=\(detail)") }
    ...
    public func database(
        _ db: Database,
        didCreate tableName: String,
        using definition: FTS5TableDefinition
    ) throws {
        switch definition.contentMode {
            case .raw: break
            case let .synchronized(contentTable):
                ...
                if definition.configuration.temporary {
                    ...
                    //
                    // sqlite> CREATE TABLE t(id INTEGER PRIMARY KEY, a, b, c); sqlite> CREATE VIRTUAL
                    // TABLE temp.ft USING fts5(content="t",content_rowid="a",b,c); sqlite> INSERT INTO
                    // ft(ft) VALUES('rebuild'); Runtime error: SQL logic error
...
public final class FTS5TableDefinition {
    ...
    /// <https://www.sqlite.org/fts5.html#fts5_table_creation_and_initialization>
    public var tokenizer: FTS5TokenizerDescriptor?

    /// The FTS5 `content` option.
    ///
    /// When you want the full-text table to be synchronized with the content of an external table,
    /// prefer the ``synchronize(withTable:)`` method.
    ///
    /// Setting this property invalidates any synchronization previously established with the
    ...
    public var content: String? {
        get {
            switch contentMode {
                case .raw(let content, _): content
                case let .synchronized(contentTable): contentTable
            }
        }
        set {
            switch contentMode {
                ...
                case .synchronized: contentMode = .raw(content: newValue, contentRowID: nil)
            }
        }
    }
    ...
    public var contentRowID: String? {
        get {
            switch contentMode {
                case .raw(_, let contentRowID): contentRowID
        ...
        set {
            switch contentMode {
                ...
                case .synchronized: contentMode = .raw(content: nil, contentRowID: newValue)
            }
        }
    }
    ...
    init(configuration: VirtualTableConfiguration) { self.configuration = configuration }
    ...
    /// - returns: A ``FTS5ColumnDefinition`` that allows you to refine the column definition.
    @discardableResult
    ...
    ///
    /// The full-text table is initially populated with the existing content in the external table.
    /// SQL triggers make sure that the full-text table is kept up to date with the external table.
    ///
...
// Explicit non-conformance to Sendable: `FTS5TableDefinition` is a mutable class and there is no
...
///
/// You get instances of `FTS5ColumnDefinition` when you create an ``FTS5`` virtual table. For
/// example:
///
/// ```swift
/// try db.create(virtualTable: "document", using: FTS5()) { t in
...
// Explicit non-conformance to Sendable: `FTS5ColumnDefinition` is a mutable class and there is no
// known reason for making it thread-safe.
@available(*, unavailable)
